...
const copy = {
  en: {
    confirmationRequired: "Type RUN to confirm this finance batch operation.",
    ...
    reasonRequired: "Enter an audit reason with at least 8 characters.",
  ...
  zh: {
    confirmationRequired: "请输入 RUN 确认执行这次财务批处理。",
    ...
    reasonRequired: "请输入至少 8 个字符的审计原因。",
...
export async function processAdminLedgerAction(
  ...
  if (reason.length < 8) {
    return { message: labels.reasonRequired, operation, status: "error" };
  }

  if (confirmation !== "RUN") {
    return { message: labels.confirmationRequired, operation, status: "error" };
  }
  ...
  try {
    const response = await fetch(`${getApiUrl()}${endpointForOperation(operation)}`, {
      body: JSON.stringify({ confirmation, limit, reason }),
      headers: {
