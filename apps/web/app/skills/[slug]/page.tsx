...
import {
  getPublicMarketplaceSkill,
  getRelatedMarketplaceSkills,
} from "@/lib/public-marketplace";
import { getPublicPaymentProviders } from "@/lib/payment-providers";
import {
...
const copy = {
  en: {
    ...
    developerPacket: {
      ...
      body: "This verified listing can become project state only after sign-in, with a version pin, policy gate, reveal-once runtime key, login-gated runtime test, and prelaunch billing or ledger evidence where applicable.",
      keyValue: "Reveal-once key",
      latest: "latest",
      policy: {
        detailHigh:
          "High-risk capabilities pause at owner approval before an agent can invoke them.",
        detailNormal:
          "Project policy records approval, budget, and runtime limits before agent use.",
      ...
      rows: {
        contract: [
          "Contract pin",
          "Schema, permissions, and runtime stay inspectable.",
        ],
        key: ["Runtime key", "Managed from the project command center."],
        ledger: [
          "Runtime evidence model",
          "Runtime evidence can feed future invoice, audit, and paid-marketplace review.",
        ],
        project: [
          "Project state",
          "Saved or installed under one organization.",
        ],
        test: [
          "Login-gated runtime test",
          "Console tests require sign-in and use the same gateway path as agent calls.",
        ],
      },
    ...
    payoutBody:
      "Paid usage is only modeled as prelaunch ledger state until payment capture and payout automation are enabled.",
    ...
    relatedBody:
      "Compare similar skills before adopting them into a signed-in project, or keep a safer replacement path ready for deprecated, suspended, or high-risk capabilities.",
    ...
    supportItems: [
      ...
      "Deprecation requires advance notice",
      "Runtime incident reporting requires sign-in and uses verified runtime path",
    ],
    trustLevels: {
      active: "Active",
      blocked: "Blocked",
  ...
  zh: {
    ...
    feedback: "用户反馈",
    feedbackBody:
      "已发布的反馈来自经过审核的登录评估。提交的技能在验证通过前不可安装。",
    ...
    developerPacket: {
      ...
      body: "此已验证列表只能在登录后成为项目状态，包含版本固定、策略门槛、一次性运行密钥、需登录的运行测试，以及预发布计费或账本证据（如适用）。",
      keyValue: "一次性密钥",
      latest: "最新",
      policy: {
      ...
      rows: {
        ...
        ledger: [
          "调用记录模型",
          "调用记录可用于未来发票、审计和付费市场审核。",
        ],
        project: ["项目状态", "保存或安装在一个组织下。"],
        test: [
          "需登录的运行测试",
          "控制台测试需要登录且使用与代理调用相同的网关路径。",
        ],
      },
      title: "已认证项目路径预览",
    },
    ...
    payout: "付费市场",
    payoutBody:
      "付费使用仅作为预发布账本状态建模，直到支付捕获和分成自动化启用。",
    permissions: "权限",
    pricing: "定价",
    ...
    related: "替代方案与替换",
    relatedBody:
      "在将类似技能采用到登录项目前进行比较，或为已弃用、暂停或高风险能力准备更安全的替换路径。",
    relatedDetails: "查看详情",
    relatedReasons: "匹配原因",
    reviews: "运营笔记",
    runtime: "运行时",
...
export default async function SkillDetailPage({
  ...
  const [{ slug }, search] = await Promise.all([params, searchParams]);
  const locale = getLocaleFromSearchParams(search);
  const [skill, projects, relatedSkills, feedbackData, session, paymentProviders] =
    await Promise.all([
  ...
  const publisherProfile = await getPublicPublisherProfile(
    publisherSlugFromName(skill.author),
  );
  const latestVersion = skill.changelog[0]?.version;
  const skillAvailability = getSkillAvailability(skill.verification.en);
  const skillActionState = getPublicSkillActionState(
  ...
  const skillLoginHref = localizedHrefWithReturnTo(
    "/login",
    locale,
    skillReturnTo,
  );
  ...
  const DeveloperAccessIcon =
    skillActionState.canShowProjectHandoff && hasDeveloperAccess
      ? WalletCards
      : KeyRound;
  ...
  const availableCommandRows = skillActionState.canShowProjectHandoff
    ? installRows
    : publicInspectRows;
  ...
  return (
    <AppShell active="skills" locale={locale}>
      <div className={"skill-detail-shell " + styles.pageStyles}>
        <Reveal>
          <section className="section pt-16 pb-12">
            <div className="section-inner">
              <a
                className="btn-text text-[#999] hover:text-white text-sm mb-4 inline-block"
              ...
              <p className="body-text text-[#999] mt-3 max-w-[720px]">
            ...
            <aside className="skill-detail-status-card bg-[#212121] border border-[rgba(255,255,255,0.08)] rounded-[16px] mt-8">
              ...
              {showProductionMetrics ? (
                <div className="skill-detail-status-card__grid grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="stat-card">
                    ...
                    <strong className="text-white">
                      {formatMetricValue(skill.successRate, locale)}
                    </strong>
                  </div>
                  <div className="stat-card">
                    <span className="text-[#999] text-xs">
                    ...
                    <strong className="text-white">
                      {formatMetricValue(skill.latency, locale)}
                    </strong>
                  </div>
                  <div className="stat-card">
                    <span className="text-[#999] text-xs">
                ...
                <div className="skill-detail-status-card__grid grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="stat-card">
                    <span className="text-[#999] text-xs">
                      {labels.previewMetric}
                    </span>
                    <strong className="text-white">
                      {labels.metricUnavailable}
                    </strong>
                  </div>
                </div>
              )}
            ...
            <div className="section-inner mt-5">
              <section
                className="skill-adoption-panel"
              ...
              >
                <div className="skill-adoption-panel__head">
                  ...
                  <a
                    className="btn-secondary skill-adoption-panel__action"
                    href={adoptionProfile.actionHref}
                  >
            ...
            <div className="flex flex-col gap-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                    {skill.securityReport.map((item) => (
                      <div
                        ...
                        key={localizeText(item.label, locale)}
                      >
                        <strong className="text-sm text-white">
                          {localizeText(item.label, locale)}
                        </strong>
                        <span className="text-sm text-[#999]">
                          {localizeText(item.value, locale)}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <Reveal delay={400}>
                <article className="card">
                  ...
                  <p className="body-text-sm text-[#999] mt-3">
                    {labels.feedbackBody}
                  </p>
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    ...
                    <div className="stat-card">
                      <strong className="text-white text-lg">
                        {feedbackData.summary.publishedCount}
                      </strong>
                      <span className="text-[#999] text-xs">
                        {labels.publishedFeedback}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-4 mt-6">
                    {feedbackData.feedback.length > 0 ? (
                      feedbackData.feedback.map((feedback) => (
                        <section
                          className="bg-black border border-[rgba(255,255,255,0.08)] rounded-[12px] p-4"
                          key={feedback.id}
                        >
                            ...
                            <div>
                              ...
                              <span className="block text-[#666] text-xs mt-0.5">
                                {feedback.reviewerOrganizationName ??
                                  ...
                                  labels.anonymousReviewer}
                              </span>
                            </div>
                            <div
                              className="flex items-center gap-0.5 text-[#7fee64]"
                          ...
                          {feedback.publisherResponseBody ? (
                            <div className="mt-3 pl-3 border-l-2 border-[rgba(255,255,255,0.08)]">
                              <strong className="text-xs text-[#666]">
                                {labels.publisherResponse}
                              </strong>
                              <p className="body-text-sm text-[#999] mt-1">
                                {feedback.publisherResponseBody}
                              </p>
                              {feedback.publisherRespondedAt ? (
                                <small className="text-xs text-[#525252] mt-1 block">
                                  {formatDate(
                                    feedback.publisherRespondedAt,
                                    locale,
                                  )}
                      ...
                      <p className="body-text-sm text-[#525252]">
                        {labels.feedbackEmpty}
                      </p>
                    )}
                  </div>
                </article>
                  ...
                  <article className="card">
                    ...
                    <div className="flex flex-col gap-4 mt-4">
                      {relatedSkills.map((suggestion) => {
                        const relatedInstallState = getSkillInstallState(
                          suggestion.skill.verification.en,
                        );

                        return (
                          <section
                            className="bg-black border border-[rgba(255,255,255,0.08)] rounded-[12px] p-4"
                            key={suggestion.skill.slug}
                          >
                            <header className="flex items-start justify-between gap-3">
                              <div>
                                <strong className="text-white text-sm">
                                  {localizeText(suggestion.skill.name, locale)}
                                </strong>
                                <span className="block text-[#666] text-xs mt-0.5">
                                  {localizeText(
                                    suggestion.skill.summary,
                                    locale,
                                  )}
                                </span>
                              </div>
                              <span
                                className={`pill ${suggestion.skill.risk === "high" ? "pill--danger" : suggestion.skill.risk === "medium" ? "pill--warning" : "pill--success"}`}
                              >
                                {formatRiskLabel(suggestion.skill.risk, locale)}
                              </span>
                            </header>
                            ...
                            <div className="flex flex-wrap gap-2 mt-3">
                              ...
                              <span className="pill pill--neutral">
                                {suggestion.skill.runtime}
                              </span>
                              <span className="pill pill--neutral">
                                {formatPublicSkillPrice(
                                  suggestion.skill.billing,
                                  locale,
                                )}
                              </span>
                              <span className="pill pill--neutral">
                                {localizeText(
                                  suggestion.skill.verification,
                                  locale,
                                )}
                              </span>
                            </div>
                            ...
                            <div
                              ...
                              aria-label={labels.relatedReasons}
                            >
                            ...
                            <div className="flex items-center justify-between gap-3 mt-3 pt-3 border-t border-[rgba(255,255,255,0.08)]">
                              <code className="text-xs text-[#999] truncate">
                                {relatedInstallState.installable
                                  ? suggestion.skill.installsCommand.cli
                                  : relatedInstallState.reason[locale]}
                              </code>
                              <a
                                className="btn-secondary shrink-0 text-xs"
                                href={localizedHref(
                                  `/skills/${suggestion.skill.slug}`,
                                  locale,
                                )}
                              >
              ...
              <aside className="flex flex-col gap-6">
                <section className="card">
                  ...
                  <div className="mt-4">
                    <strong className="text-white block">
                      {publisherProfile?.displayName ?? skill.author}
                    </strong>
                    <span className="text-xs text-[#666] block mt-1">
                      {labels.publisherTrust}
                    </span>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {publisherProfile ? (
                        <span
                          className={`pill ${publisherProfile.trustLevel === "verified" ? "pill--success" : publisherProfile.trustLevel === "blocked" ? "pill--danger" : publisherProfile.trustLevel === "limited" ? "pill--warning" : "pill--neutral"}`}
                        >
                          {labels.trustLevels[publisherProfile.trustLevel]}
                        </span>
                      ) : null}
                      {publisherProfile ? (
                        <span className="pill pill--neutral">
                          {publisherProfile.metrics.verifiedSkillCount}{" "}
                          {labels.verifiedSkills}
                        </span>
                      ) : null}
                    </div>
                    {publisherProfile ? (
                      <a
                        className="btn-text text-sm mt-3 inline-flex items-center gap-1.5"
                        href={localizedHref(
                          `/publishers/${publisherProfile.slug}`,
                          locale,
                        )}
                      >
                ...
                <section className="card">
                  ...
                  <div className="mt-4">
                    ...
                    <span className="text-sm text-[#999] block mt-1">
                      {pricingPreviewBody(
                        skill.billing,
                        skillAvailability.kind,
                        locale,
                      )}
                    </span>
                  </div>
                ...
                <section className="card">
                  ...
                  <div className="flex flex-col gap-3 mt-4">
                    {skill.permissions.map((permission) => (
                      ...
                      >
                        <strong className="text-sm text-white">
                          {localizeText(permission.label, locale)}
                        </strong>
                        <span className="text-xs text-[#999]">
                          {localizeText(permission.value, locale)}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <section className="card">
                  ...
                  {skill.reviews.map((review) => (
                    <blockquote
                      className="mt-4 pl-3 border-l-2 border-[rgba(255,255,255,0.08)]"
                      key={review.author}
                    >
                      <p className="body-text-sm text-[#999] italic">
                        {localizeText(review.quote, locale)}
                      </p>
                      <cite className="block text-xs text-[#525252] mt-1 not-italic">
                        {review.author}
                      </cite>
                    </blockquote>
                  ))}
                ...
                <section className="card">
                  ...
                  <div className="flex flex-col gap-2 mt-4">
                    {labels.supportItems.map((item) => (
                      <span className="body-text-sm text-[#999]" key={item}>
                        {item}
                      </span>
                    ))}
                  </div>
...
function DeveloperHandoffPacket({
  ...
  const rows = [
    ...
    },
    {
      ...
      value:
        projectCount > 0
          ? `${projectCount} · ${labels.projectReady}`
          : labels.projectMissing,
    },
    {
      detail:
        risk === "high" ? labels.policy.detailHigh : labels.policy.detailNormal,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-[rgba(255,255,255,0.08)]">
        {rows.map((row) => (
          <div className="bg-[#212121] p-4 flex flex-col gap-1" key={row.label}>
            ...
            <span
              className={`pill ${row.tone === "warning" ? "pill--warning" : "pill--success"} w-fit`}
            >
              {row.label}
            </span>
...
function getSkillAdoptionProfile({
  ...
  const firstUseCase = skill.useCases[0]
    ? localizeText(skill.useCases[0], locale)
    : "";
  ...
  return {
    actionHref: skillActionState.canShowProjectHandoff
      ? localizedHref(developerAccessHref, locale)
      : "#install",
    ...
    items: [
      {
        ...
        value: isInspectionOnly
          ? skillAvailability.label[locale]
          : formatBillingModelLabel(skill.billing, locale),
      },
      {
    ...
    kicker: isInspectionOnly
      ? labels[locale].publicInspection
      : labels[locale].verifiedRuntime,
...
function getCategoryBuyerFitBody(
  ...
  const labels = {
    en: {
      automation:
        "Best when the same judgment or routing step repeats across many tasks.",
      content:
        "Use when draft quality, positioning, tone, and conversion clarity matter.",
      ...
      ecommerce:
        "Use for catalog, listing, pricing, inventory, and promotion workflows.",
      education:
        "Use to turn materials into structured learning or enablement assets.",
      finance:
        "Use when back-office teams need extracted, checked, or routed financial data.",
      hr: "Use where recruiting or people workflows need consistent screening and summaries.",
      legal:
        "Use for review support, policy checks, and structured compliance summaries.",
      marketing:
        "Use when campaigns need repeatable checks, briefs, and performance actions.",
      ops: "Use when support queues, knowledge bases, and internal operations need faster triage.",
      research:
        "Use when evidence gathering, source review, or browser research must stay structured.",
      sales:
        "Use when pipeline records, outreach, and account notes need sharper next actions.",
      security:
        "Use when agent actions must be gated, reviewed, or risk-scored before execution.",
...
function getSkillPreparationTitle(
  ...
  if (skill.runtime === "Local") {
    return locale === "zh"
      ? "准备本地文件或项目上下文"
      : "Prepare local files or project context";
  }

  if (skill.runtime === "MCP") {
    return locale === "zh"
      ? "准备 MCP 连接和项目 Key"
      : "Prepare MCP connection and project key";
  }

  return locale === "zh"
    ? "准备输入和项目 Key"
    : "Prepare input and project key";
}
...
function formatBillingModelLabel(
  ...
  const labels = {
    ...
    Locale,
    Record<"free" | "per_call" | "subscription", string>
  >;
...
function SkillInspectionOnlyNotice({ locale }: { locale: Locale }) {
  return (
    <div className="mt-4 p-4 border border-[rgba(255,255,255,0.08)] rounded-[12px] flex flex-col gap-2">
      ...
      <strong className="text-white text-sm">
        {locale === "zh" ? "仅可查看" : "Inspection only"}
      </strong>
