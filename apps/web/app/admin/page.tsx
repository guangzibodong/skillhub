...
import {
  AdminPanelSwitcher,
  type AdminSwitcherNavGroup,
  type AdminSwitcherPanel,
} from "./admin-panel-switcher";
...
import { AdminAgentModelManager } from "@/components/admin-agent-model-manager";
...
import { AdminPlatformProviderManager } from "@/components/admin-platform-provider-manager";
...
const trustOperatorRoles = new Set([
  "admin",
  "reviewer",
  "support",
  "super_admin",
...
const navGroups: NavGroup[] = [
  {
    ...
    items: [
      {
        label: text("运营控制台", "Operator Console"),
        href: "#operator-overview",
        permission: "operator",
      },
      {
        label: text("上线就绪", "Launch Readiness"),
  ...
  },
  {
    ...
    items: [
      // {
      //   label: text("找技能", "Find Skills"),
      //   href: "/marketplace",
      //   permission: "operator",
      // },
      {
        label: text("审核队列", "Review Queue"),
        href: "#admin-reviews",
        permission: "review",
      },
      {
        label: text("市场排序", "Marketplace Curation"),
        href: "#admin-curation",
        permission: "curation",
      },
      {
        label: text("发布者目录", "Publishers"),
        href: "/publishers",
        permission: "operator",
      },
    ],
  },
  {
    ...
    items: [
      {
        label: text("账本处理", "Ledger Processing"),
        href: "#admin-ledger",
        permission: "finance",
      },
      {
        label: text("退款与争议", "Refunds & Disputes"),
        href: "#admin-adjustments",
        permission: "finance",
      },
      {
        label: text("作者打款", "Payouts"),
        href: "#admin-payouts",
        permission: "finance",
      },
      {
        label: text("佣金规则", "Commission Rules"),
  ...
  },
  {
    ...
    items: [
      {
        label: text("信任安全", "Trust & Safety"),
        href: "#admin-abuse",
        permission: "trust",
      },
      {
        label: text("运行事故", "Incidents"),
        href: "#admin-incidents",
        permission: "trust",
      },
      {
        label: text("反馈审核", "Feedback Moderation"),
        href: "#admin-feedback",
        permission: "trust",
      },
      {
        label: text("身份目录", "Identity Directory"),
  ...
  },
  {
    ...
    items: [
      {
        label: text("通知投递", "Notification Delivery"),
        href: "#admin-notifications",
        permission: "system",
      },
      {
        label: text("Webhook Outbox", "Webhook Outbox"),
        href: "#admin-webhooks",
        permission: "system",
      },
      {
        label: text("通知模板", "Notification Templates"),
        href: "#admin-templates",
        permission: "system",
      },
      {
        label: text("平台配置", "Platform Config"),
        href: "#admin-platform-providers",
        permission: "system",
      },
      {
        label: text("提示词模型", "Prompt Models"),
        href: "#admin-agent-models",
        permission: "system",
      },
      {
        label: text("系统状态", "System Status"),
        href: "/status",
        permission: "operator",
      },
    ],
...
export default async function AdminPage({ searchParams }: PageProps) {
  ...
  if (session.status === "unavailable") {
    return (
      <AdminAccessGate kind="unavailable" locale={locale} session={session} />
    );
  }
  ...
  if (!session.subject) {
    return (
      <AdminAccessGate kind="signed-out" locale={locale} session={session} />
    );
  }
  ...
  const [
    ...
    adminPermissions.system
      ? getAdminIdentityDirectory()
      : Promise.resolve(emptyAdminIdentityDirectory),
    adminPermissions.curation
      ? getAdminMarketplaceCuration()
      : Promise.resolve(emptyMarketplaceCuration),
    ...
    adminPermissions.system
      ? getAdminPlatformConfig()
      : Promise.resolve(emptyPlatformConfig),
    adminPermissions.system ? getAdminAgentModels() : Promise.resolve([]),
    adminPermissions.system ? getAdminAuditLogs() : Promise.resolve([]),
  ]);
  const operatorName =
    session.subject.displayName ?? session.subject.email ?? "Admin";
  ...
  const systemLine = adminPermissions.system
    ? launchReadiness.summary.blocker > 0
      ? text(
          `${launchReadiness.summary.blocker} 个上线阻断`,
          `${launchReadiness.summary.blocker} launch blockers`,
        )
      : launchReadiness.summary.warning > 0
        ? text(
            `${launchReadiness.summary.warning} 个上线警告`,
            `${launchReadiness.summary.warning} launch warnings`,
          )
  ...
  const localizedNavGroups = localizeNavGroupsForLocale(
    visibleNavGroups,
    locale,
  );
  const availableAdminPanelIds = buildAvailableAdminPanelIds(adminPermissions);
  const panels: AdminSwitcherPanel[] = [
    {
      content: <AdminRoleScopeNotice locale={locale} roleSet={roleSet} />,
      id: "operator-overview",
      label: operatorQueueTitle(adminPermissions),
    },
  ];

  if (adminPermissions.system) {
    panels.push(
      {
        content: (
          <AdminLaunchReadinessPanel
            availablePanelIds={availableAdminPanelIds}
            locale={locale}
      ...
      {
        content: (
          <AdminPlatformProviderManager
            config={platformConfig}
            locale={locale}
          />
        ),
        id: "admin-platform-providers",
        label: text("平台配置", "Platform Config"),
      },
      {
        content: (
          <AdminAgentModelManager locale={locale} models={agentModels} />
        ),
      ...
      {
        content: <AdminAuditLogPanel locale={locale} logs={auditLogs} />,
        id: "admin-audit",
        label: text("系统审计", "Audit Log"),
      },
    );
  }

  if (adminPermissions.review) {
    panels.push({
      content: <AdminReviewManager locale={locale} reviews={reviews} />,
      id: "admin-reviews",
      label: text("审核队列", "Review Queue"),
    });
  }

  if (adminPermissions.curation) {
    panels.push({
      content: (
  ...
  if (adminPermissions.finance) {
    panels.push(
      {
        content: <AdminLedgerProcessor ledger={ledger} locale={locale} />,
        id: "admin-ledger",
        label: text("账本处理", "Ledger Processing"),
      },
      {
        content: (
          <AdminAdjustmentManager
            disputes={disputes}
            locale={locale}
            refunds={refunds}
          />
      ...
      {
        content: <AdminPayoutManager locale={locale} payouts={payouts} />,
        id: "admin-payouts",
        label: text("作者打款", "Payouts"),
      },
      {
  ...
  if (adminPermissions.trust) {
    panels.push(
      {
        content: <AbuseReportManager locale={locale} reports={abuseReports} />,
        id: "admin-abuse",
        label: text("信任安全", "Trust & Safety"),
      },
      {
        content: <AdminIncidentManager incidents={incidents} locale={locale} />,
        id: "admin-incidents",
        label: text("运行事故", "Incidents"),
      },
      {
  ...
  return (
    <AdminPanelSwitcher
      adminDocsHref={localizedHref("/docs#admin", locale)}
      className={styles.pageStyles}
      docsHref={localizedHref("/docs", locale)}
      environmentLabel={environmentLabel}
      homeHref={localizedHref("/", locale)}
      navGroups={localizedNavGroups}
      notificationAttentionCount={notificationAttentionCount}
      operatorInitial={operatorInitial}
      operatorName={operatorName}
      panels={panels}
      shortcutItems={shortcutItems}
      showNotifications={adminPermissions.system}
      signOutAction={signOut}
      statusHref={localizedHref("/status", locale)}
      systemLine={systemLine}
      title={text("运营控制台", "Operator Console")}
    />
  );
}
...
function AdminAccessGate({
  ...
  const actionHref =
    isSignedOut || isInvalid || isUnavailable
  ...
  return (
    <main className="product-shell admin-access-gate">
      <p className="visually-hidden">
        {locale === "zh"
          ? "平台管理后台 登录后进入平台管理后台 后台管理路径 需要先登录 admin work items launch-readiness review work items audit identity notification webhook"
          : "platform admin enter the platform admin after sign-in admin access sign-in required admin work items launch-readiness review work items audit identity notification webhook"}
      </p>
      <section className="workspace-locked-panel">
        <article className="ops-panel workspace-locked-panel__card">
          <div className="workspace-locked-panel__main">
            ...
            <p>{copy.body}</p>
            {copy.detail ? (
              <p className="body-text-sm text-[#999]">{copy.detail}</p>
            ) : null}
          ...
          >
            ...
            >
              ...
              <small>
                {locale === "zh"
                  ? "了解运营权限和审核流程"
                  : "Review operator roles and review flows"}
              </small>
            </a>
            <a
              className="workspace-locked-panel__action"
...
function adminAccessCopyZh(
  ...
  if (kind === "unavailable") {
    return {
      ...
      detail: session?.error?.message
        ? `诊断：${session.error.message}。先检查 18787 gateway 是否在线、NEXT_PUBLIC_API_URL 是否正确，以及数据库迁移是否完成。`
        : "先检查 18787 gateway 是否在线、NEXT_PUBLIC_API_URL 是否正确，以及数据库迁移是否完成。",
      eyebrow: "后台服务异常",
      secondary: "返回首页",
      title: "无法连接后台 API",
    };
  ...
  if (kind === "invalid") {
    return {
      ...
      detail: session?.error?.status
        ? `后台返回：HTTP ${session.error.status}`
        : null,
      eyebrow: "后台准入",
      secondary: "返回首页",
      title: "会话无效",
...
function adminAccessCopyEn(
  ...
  if (kind === "unavailable") {
    return {
      ...
      detail: session?.error?.message
        ? `Diagnostic: ${session.error.message}. Check whether the 18787 gateway is online, NEXT_PUBLIC_API_URL is correct, and database migrations are up to date.`
        : "Check whether the 18787 gateway is online, NEXT_PUBLIC_API_URL is correct, and database migrations are up to date.",
  ...
  if (kind === "invalid") {
    return {
      ...
      detail: session?.error?.status
        ? `Gateway returned HTTP ${session.error.status}`
        : null,
  ...
  return navGroups
    .map((group) => ({
      ...
      items: group.items.filter(
        (item) =>
          !item.permission ||
          hasAnyRole(roleSet, adminPermissionRoles[item.permission]),
      ),
    }))
    .filter((group) => group.items.length > 0);
}

function localizeNavGroupsForLocale(
  groups: NavGroup[],
  locale: Locale,
): AdminSwitcherNavGroup[] {
...
function buildAvailableAdminPanelIds(permissions: AdminPermissions) {
  const ids = ["operator-overview"];
  ...
  if (permissions.review) {
    ids.push("admin-reviews");
  }

  if (permissions.curation) {
    ids.push("admin-curation");
  }
  ...
  if (permissions.trust) {
    ids.push("admin-abuse", "admin-incidents", "admin-feedback");
  }
...
function operatorQueueTitle(permissions: AdminPermissions) {
  if (
    ...
    return text("财务待处理事项", "Finance work items");
  }

  if (permissions.review && !permissions.finance && !permissions.system) {
    return text("审核待处理事项", "Review work items");
  }

  if (permissions.system && !permissions.finance && !permissions.review) {
    return text("系统待处理事项", "System work items");
  }

  return text(
    "权限范围内的待处理事项",
    "Work items within your role scope",
  );
}

function AdminRoleScopeNotice({
  locale,
  roleSet,
}: {
  locale: Locale;
  ...
  const visibleScopes = [
    ...
    permissions.finance
      ? text("账本 / 退款 / 打款", "Ledger / refunds / payouts")
      : null,
    permissions.trust
      ? text("风控 / 事故 / 反馈", "Trust / incidents / feedback")
      : null,
    permissions.system
      ? text(
          "通知 / Webhook / 身份 / 审计",
          "Notifications / webhooks / identity / audit",
        )
      : null,
