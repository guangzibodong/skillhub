...
import { PublisherWorkspaceTabs } from "@/components/publisher-workspace-tabs";
...
import {
  getPublisherPageCopy,
  type PublisherPageCopy,
} from "@/lib/publisher-page-copy";
import { buildNoIndexMetadata } from "@/lib/seo";
import { getPublicApiUrl, getPublicAppUrl } from "@/lib/api-url";
...
type PublisherWorkflowTone = "amber" | "coral" | "cyan" | "signal" | "neutral";

type PublisherWorkflowStep = {
  body: string;
  label: string;
  tone: PublisherWorkflowTone;
  title: string;
};
...
const publisherCommandCopy = {
  en: {
    body: "Start with the highlighted task, then use the shortcuts below to publish, manage skills, complete account setup, review payout readiness, or respond to demand.",
    completeAction: "Review publisher operations",
    completeDetail:
      "Core publisher readiness is complete. Monitor review check results, buyer demand, paid-marketplace ledger state, feedback, finance-gated payout readiness, and marketplace placement.",
    completeTitle: "Keep the supply loop healthy",
    eyebrow: "Next step",
    ready: "Ready",
    title: "Next action for this workspace",
    actions: {
      payout: "Open paid-readiness information",
    ...
    queue: {
      ...
      readyDetail:
        "No urgent publisher blockers are visible. Keep watching reviews, feedback, buyer demand, paid-marketplace ledger state, finance-gated payout readiness, and marketplace placement.",
    ...
    queueActions: {
      ...
      runtime: "Open review check results",
    },
    queueItems: {
      adjustments: "Refund or dispute needs a publisher response",
      demand: "Buyer request is waiting for a publisher action",
      distribution:
        "Marketplace placement needs improvement or appeal follow-up",
      feedback: "Published buyer feedback needs a publisher response",
      payoutReady: "Paid marketplace payout review is ready",
      payoutBlocked: "Paid marketplace payout readiness is blocked",
      pricing: "Paid marketplace review is blocked",
    ...
    queueTitles: {
      ...
      payout: "Paid marketplace payout readiness",
      pricing: "Paid marketplace review",
  ...
  zh: {
    body: "先处理下面标出的下一步；常用入口在下方：发布新技能、管理技能、完善账号、查看收款、响应需求。",
    completeAction: "\u67e5\u770b\u53d1\u5e03\u8005\u8fd0\u8425",
    completeDetail:
      "核心发布准备已完成。接下来重点监控审核检查结果、买方需求、预发布账本、反馈、收款资料和市场位置。",
...
const copy = {
    ...
    readiness: {
      ...
      tasks: {
        session: [
          ...
          "Sign in through the account entry so publishing, pricing intent, paid-readiness information, and notifications stay scoped to the current organization.",
        ],
        profile: [
        ...
        verified: [
          ...
          "Complete review and activate pricing so buyers can trust and install the skill.",
        ],
        payout: [
  ...
  zh: {
    ...
    disputeReview: "争议复核",
    description:
      "给技能发布者使用的独立工作台：推进技能审核，记录未来付费市场复核所需的定价意图，响应买方需求，并准备付费市场准备信息。",
    eyebrow: "发布者工作台",
    ...
    readiness: {
      ...
      tasks: {
        session: [
          "登录工作区会话",
          "通过账号入口登录，让发布、定价意图、付费准备和通知都归属到当前组织。",
        ],
        profile: [
          "创建发布者档案",
...
function PublisherTabWorkflow({
  ariaLabel,
  steps,
}: {
  ariaLabel: string;
  steps: PublisherWorkflowStep[];
}) {
  return (
    <section className="publisher-tab-workflow" aria-label={ariaLabel}>
      {steps.map((step, index) => (
        <div
          className={
            "publisher-tab-workflow__step publisher-tab-workflow__step--" +
            step.tone
          }
          key={step.title + "-" + index}
        >
          <span className="publisher-tab-workflow__index">
  ...
  return (
    <section className="workspace-locked-panel">
      <article className="ops-panel workspace-locked-panel__card">
        ...
        <div
          className="workspace-locked-panel__actions"
          aria-label={guide.ariaLabel}
        >
          {guide.actions.map((action, index) => {
            const Icon =
              [BadgeCheck, ClipboardList, PackageCheck][index] ?? CircleDot;

            return (
              <a
                className="workspace-locked-panel__action"
...
function getPublisherLockedGuide(locale: Locale) {
  if (locale === "zh") {
    return {
      ariaLabel: "发布者准入步骤",
      eyebrow: "发布者准入",
      marker: "发布者工作台 / 优先级队列 / 付费准备信息 / 收款资料",
      actions: [
        {
  ...
  return {
    ...
    marker:
      "publisher next actions / review readiness / paid marketplace preview",
    actions: [
      {
...
function buildReadinessTasks(
  ...
  return [
    ...
    },
    {
      ...
      id: "terms",
      status: statusFor(
        flags.hasAcceptedCurrentTerms,
        flags.hasPublisherProfile,
      ),
    ...
    },
    {
      ...
      id: "publish",
      status: statusFor(
        flags.hasPublishedSkill,
        flags.hasPublisherProfile && flags.hasAcceptedCurrentTerms,
      ),
    ...
    },
    {
      ...
      id: "verified",
      status: statusFor(
        flags.hasActiveVerifiedListing,
        flags.hasPublishedSkill,
      ),
...
function buildPublisherPriorityItems({
  ...
  if (currentReadinessTask) {
    items.push({
      ...
      metric:
        currentReadinessTask.status === "blocked"
          ? commandLabels.queue.blockedMetric
          : commandLabels.queue.nextMetric,
  ...
  const reviewRepairCount = skills.filter((skill) => {
    ...
    return (
      ...
      (hasVersion &&
        ...
        !["queued", "in_review", "approved"].includes(reviewStatus))
    );
  }).length;
  const failedRuntimeCount = skills.filter((skill) =>
    getPublisherRuntimeChecks(skill).some(
      (check) => check.status === "failed" || check.isBlocking === true,
    ),
  ).length;
  ...
  const reviewSlaCount = skills.filter((skill) => {
    const slaStatus =
      skill.versions?.[0]?.reviewSlaStatus ??
      skill.review.reviewSlaStatus ??
      null;
  ...
  const feedbackResponseCount = skills.reduce(
    (count, skill) =>
      count +
      (skill.recentFeedback ?? []).filter(
        (feedback) => !feedback.publisherResponseBody,
      ).length,
    0,
  );
  const distributionActionCount = skills.filter((skill) => {
    const activeAppeal =
      ...
      ["open", "under_review"].includes(skill.marketplace.appeal.status);
  ...
  if (failedRuntimeCount > 0 || warningRuntimeCount > 0) {
    ...
    items.push({
      ...
      href: localizedHref("/publisher#publisher-skills", locale),
      id: "runtime-checks",
      metric: hasFailedChecks
        ? formatCompactNumber(failedRuntimeCount)
        : formatCompactNumber(warningRuntimeCount),
  ...
  if (payoutNeedsAttention) {
    items.push({
      ...
      detail: payoutReadiness?.canRequest
        ? commandLabels.queueItems.payoutReady
        : commandLabels.queueItems.payoutBlocked,
      href: localizedHref("/publisher#publisher-payout", locale),
      id: "payout",
      metric: payoutReadiness?.canRequest
        ? commandLabels.queueTones.ready
        : formatCompactNumber(payoutBlockerCount),
  ...
  if (items.length === 0) {
    items.push({
      actionLabel:
        skillCount > 0
          ? commandLabels.queueActions.monitor
          : commandLabels.actions.publish,
      detail:
        skillCount > 0 ? commandLabels.queue.readyDetail : commandLabels.body,
      href:
        skillCount > 0
          ? localizedHref("/publisher#publisher-skills", locale)
          : localizedHref("/publish", locale),
      id: "healthy-supply-loop",
      metric: commandLabels.queue.readyMetric,
      priority: 100,
      title:
        skillCount > 0
          ? commandLabels.queue.readyTitle
          : commandLabels.actions.publish,
      tone: "ready",
    });
  }
...
function getPublisherRuntimeChecks(skill: PublisherSkillRecord) {
  return skill.versions?.[0]?.runtimeChecks?.length
    ? skill.versions[0].runtimeChecks
...
function formatLedgerStatus(value: string, labels: PublisherPageCopy) {
  return (
    labels.ledgerStatuses[value as keyof typeof labels.ledgerStatuses] ??
    value.replaceAll("_", " ")
  );
}
...
function formatLedgerSource(
  ...
  return (
    labels.ledgerSources[value as keyof typeof labels.ledgerSources] ??
    value.replaceAll("_", " ")
  );
}
...
function formatLedgerSourceReference(value: string | null | undefined) {
  ...
  if (value.startsWith("subscription:")) {
    ...
    const subscriptionTail =
      subscriptionId.length > 8 ? subscriptionId.slice(0, 8) : subscriptionId;

    return periodDate
      ? `sub ${subscriptionTail} / ${periodDate}`
      : `sub ${subscriptionTail}`;
  }
...
function formatRefundStatus(value: string, labels: PublisherPageCopy) {
  return (
    labels.refundStatuses[value as keyof typeof labels.refundStatuses] ??
    value.replaceAll("_", " ")
  );
}
...
function formatDisputeStatus(value: string, labels: PublisherPageCopy) {
  return (
    labels.disputeStatuses[value as keyof typeof labels.disputeStatuses] ??
    value.replaceAll("_", " ")
  );
}

function buildCommercialSkillRows(
  skills: PublisherSkillRecord[],
): CommercialSkillRow[] {
  return skills
    .filter(
    ...
    .map((skill) => ({
      ...
      ready: Boolean(
        skill.commercial?.paidActivationReady &&
        skill.pricing.billingModel !== "free" &&
...
function formatCommercialState(
  ...
  return (
    labels.commercial.states[value as keyof typeof labels.commercial.states] ??
    value.replaceAll("_", " ")
  );
}

function formatCommercialPrice(
  row: CommercialSkillRow,
  labels: PublisherPageCopy,
) {
...
function formatCommercialAction(
  blockers: PublisherCommercialBlocker[],
  labels: PublisherPageCopy,
) {
  ...
  return blockers
    ...
    .map(
      (blocker) =>
        labels.commercial.actionLabels[blocker] ?? blocker.replaceAll("_", " "),
    )
    .join(" / ");
}
...
export default async function PublisherPage({ searchParams }: PageProps) {
  ...
  const hasPublisherAccess =
    hasWorkspaceSession &&
    publisherAccessRoles.some((role) => roleSet.has(role));
  const publisherLoginPath = hrefWithReturnTo("/login", "/publisher", locale);
  const shellSecondaryHref = hasWorkspaceSession
    ? localizedHref("/account", locale)
    : undefined;
  const shellSecondaryLabel = hasWorkspaceSession
    ? locale === "zh"
      ? "个人中心"
      : "Account"
    : undefined;
      ...
      <main className={`product-shell ${styles.pageStyles}`}>
        ...
        <WorkspaceLockedPanel
          actionHref={
            hasWorkspaceSession
              ? localizedHref("/account", locale)
              : localizedHrefWithReturnTo("/login", locale, "/publisher")
          }
          actionLabel={
            hasWorkspaceSession
              ? locale === "zh"
          ...
          body={
            locale === "zh"
              ? "发布者工作台包含草稿、审核、定价意图、付费准备信息、收款资料和反馈操作。当前账号还没有发布权限，所以先显示准入步骤，表单和工作区数据保持隐藏。"
  ...
  const verifiedSkillCount = publisherSkills.filter(
    (skill) => skill.verificationStatus === "verified",
  ).length;
  const activeDemandCount = countActivePublisherRequests(
    publisherBuyerRequests,
  );
  const visibleMetrics = [
    ...
    [
      ...
      formatMoney(financeLedger.summary.availableBalanceCents),
    ],
  ...
  const hasVerifiedPayoutAccount =
    publisherAccount.payoutAccounts.some(
      (account) => account.status === "verified",
    ) ||
    payoutSummary.payoutAccounts.some(
      (account) => account.status === "verified",
    );
  ...
  const draftPaidPrices = publisherSkills.filter(
    (skill) =>
      skill.pricing.billingModel !== "free" && skill.pricing.status === "draft",
  ).length;
  ...
  const commercialMetrics = [
    [
      labels.commercial.metrics.readyPaid,
      formatCompactNumber(readyPaidListings),
    ],
    [
      labels.commercial.metrics.blockedPaid,
      formatCompactNumber(blockedPaidListings),
    ],
    [labels.commercial.metrics.draftPaid, formatCompactNumber(draftPaidPrices)],
    [
      labels.commercial.metrics.payout,
      hasPayoutReady
        ? labels.commercial.ready
  ...
  const readinessDone = readinessTasks.filter(
    (task) => task.status === "done",
  ).length;
  const readinessPercent = Math.round(
    (readinessDone / readinessTasks.length) * 100,
  );
  ...
  const adjustmentActionCount = countPublisherAdjustmentActions(
    publisherRefunds,
    publisherDisputes,
  );
  ...
  const publisherCommandMetrics = [
    [commandLabels.metrics.readiness, String(readinessPercent) + "%"],
    ...
    [
      commandLabels.metrics.payout,
      hasPayoutReady
        ? commandLabels.ready
        : formatCommercialState(payoutStatus, labels),
    ],
  ];
  const workflowLabels = labels.workspaceFlow;
  const workflowStates = workflowLabels.states;
  const skillWorkflowSteps: PublisherWorkflowStep[] = [
    {
      body: workflowLabels.skills.steps[0][1],
      label:
        publisherSkills.length > 0
          ? workflowStates.done
          : workflowStates.current,
      title: workflowLabels.skills.steps[0][0],
      tone: publisherSkills.length > 0 ? "signal" : "cyan",
    },
    {
      body: workflowLabels.skills.steps[1][1],
      label: reviewWorkCount > 0 ? workflowStates.current : workflowStates.done,
      title: workflowLabels.skills.steps[1][0],
      tone: reviewWorkCount > 0 ? "coral" : "signal",
    },
    {
      body: workflowLabels.skills.steps[2][1],
      label:
        reviewWorkCount > 0
          ? workflowStates.current
          : publisherSkills.length > 0
      ...
      title: workflowLabels.skills.steps[2][0],
      tone:
        reviewWorkCount > 0
          ? "amber"
          : publisherSkills.length > 0
    ...
    {
      body: workflowLabels.skills.steps[3][1],
      label:
        blockedPaidListings > 0
          ? workflowStates.current
          : hasPayoutReady
            ? workflowStates.ready
            : workflowStates.monitor,
      title: workflowLabels.skills.steps[3][0],
      tone:
        blockedPaidListings > 0 ? "amber" : hasPayoutReady ? "signal" : "cyan",
    },
  ];
  const accountWorkflowSteps: PublisherWorkflowStep[] = [
    {
      body: workflowLabels.account.steps[0][1],
      label: hasPublisherProfile ? workflowStates.done : workflowStates.current,
      title: workflowLabels.account.steps[0][0],
      tone: hasPublisherProfile ? "signal" : "cyan",
    },
    {
      body: workflowLabels.account.steps[1][1],
      label: hasAcceptedCurrentTerms
        ? workflowStates.done
        : workflowStates.current,
      title: workflowLabels.account.steps[1][0],
      tone: hasAcceptedCurrentTerms ? "signal" : "amber",
    },
    {
      body: workflowLabels.account.steps[2][1],
      label: hasVerifiedPayoutAccount
        ? workflowStates.done
        : workflowStates.current,
      title: workflowLabels.account.steps[2][0],
      tone: hasVerifiedPayoutAccount ? "signal" : "cyan",
    },
    {
      body: workflowLabels.account.steps[3][1],
      label: hasPayoutReady ? workflowStates.ready : workflowStates.blocked,
      title: workflowLabels.account.steps[3][0],
      tone: hasPayoutReady ? "signal" : "amber",
    },
  ];
  const payoutCanRequest = Boolean(payoutSummary.readiness?.canRequest);
  const payoutWorkflowSteps: PublisherWorkflowStep[] = [
    {
      body: workflowLabels.payout.steps[0][1],
      label:
        blockedPaidListings > 0 || !hasPayoutReady
          ? workflowStates.current
          : workflowStates.done,
      title: workflowLabels.payout.steps[0][0],
      tone: blockedPaidListings > 0 || !hasPayoutReady ? "amber" : "signal",
    },
    {
      body: workflowLabels.payout.steps[1][1],
      label:
        financeLedger.summary.availableBalanceCents > 0
          ? workflowStates.ready
          : workflowStates.monitor,
      title: workflowLabels.payout.steps[1][0],
      tone:
        financeLedger.summary.availableBalanceCents > 0 ? "cyan" : "neutral",
    },
    {
      body: workflowLabels.payout.steps[2][1],
      label: payoutCanRequest ? workflowStates.current : workflowStates.blocked,
      title: workflowLabels.payout.steps[2][0],
      tone: payoutCanRequest ? "cyan" : "coral",
    },
    {
      body: workflowLabels.payout.steps[3][1],
      label:
        adjustmentActionCount > 0
          ? workflowStates.current
          : workflowStates.monitor,
      title: workflowLabels.payout.steps[3][0],
      tone: adjustmentActionCount > 0 ? "amber" : "neutral",
    },
  ];
  const demandWorkflowSteps: PublisherWorkflowStep[] = [
    {
      body: workflowLabels.demand.steps[0][1],
      label:
        activeDemandCount > 0 ? workflowStates.current : workflowStates.ready,
      title: workflowLabels.demand.steps[0][0],
      tone: activeDemandCount > 0 ? "amber" : "signal",
    },
    {
      body: workflowLabels.demand.steps[1][1],
      label:
        userNotificationInbox.summary.unread > 0
          ? workflowStates.current
          : workflowStates.monitor,
      title: workflowLabels.demand.steps[1][0],
      tone: userNotificationInbox.summary.unread > 0 ? "cyan" : "neutral",
    },
    {
      body: workflowLabels.demand.steps[2][1],
      label: workflowStates.monitor,
      title: workflowLabels.demand.steps[2][0],
      tone: "neutral",
    },
  ];
  const usageGrossCents = financeLedger.summary.usageGrossCents ?? 0;
  const usagePublisherShareCents =
    financeLedger.summary.usagePublisherShareCents ?? 0;
  const usageTransactionCount =
    financeLedger.summary.usageTransactionCount ?? 0;
  const subscriptionGrossCents =
    financeLedger.summary.subscriptionGrossCents ?? 0;
  const subscriptionPublisherShareCents =
    financeLedger.summary.subscriptionPublisherShareCents ?? 0;
  const subscriptionTransactionCount =
    financeLedger.summary.subscriptionTransactionCount ?? 0;
  ...
  const sourceMixRows = [
    ...
    ...source,
    sharePercent:
      sourceMixGrossCents > 0
        ? Math.round((source.grossCents / sourceMixGrossCents) * 100)
        : 0,
  }));
  const ledgerRows =
  ...
  const adjustmentRows = [
    ...
    ...publisherDisputes.slice(0, 4).map((dispute) => ({
      ...
      reason:
        dispute.reason ?? dispute.externalReference ?? labels.disputeReview,
  ...
  return (
    <main className={`product-shell ${styles.pageStyles}`}>
      ...
      {hasPublisherAccess ? (
        <PublisherWorkspaceTabs
          locale={locale}
          skillsPanel={
            <>
              <PublisherTabWorkflow
                ariaLabel={workflowLabels.skills.label}
                steps={skillWorkflowSteps}
              />
              <section
                className="publisher-priority-board"
                aria-labelledby="publisher-priority-heading"
              >
                <article className="publisher-priority-card">
                  <div className="publisher-priority-card__main">
                    ...
                    <div className="publisher-priority-title-row">
                      <h2 id="publisher-priority-heading">
                        {commandLabels.title}
                      </h2>
                      <InlineHelp
                        content={commandLabels.body}
                        label={
                          locale === "zh"
                            ? "查看任务说明"
                            : "View task guidance"
                    ...
                    <div
                      className="publisher-priority-list"
                      aria-label={commandLabels.queue.title}
                    >
                      {publisherPriorityItems.map((item) => (
                        <a
                          className={`publisher-priority-task publisher-priority-task--${item.tone}`}
                          href={item.href}
                          key={item.id}
                          title={item.detail}
                        >
                          <span>
                            {commandLabels.queueTones[item.tone]} /{" "}
                            {item.metric}
                          </span>
              ...
              <section className="publisher-command-layout publisher-command-layout--skills">
                <div className="publisher-command-main">
                  <div className="publisher-anchor-shell">
                    <PublisherSkillManager
                      locale={locale}
                      skills={publisherSkills}
                    />
                  </div>
                ...
                <aside className="publisher-command-side">
                  <article className="ops-panel publisher-launch-checklist">
                    ...
                    <div
                      className="publisher-launch-checklist__bar"
                      aria-hidden="true"
                    >
                    ...
                    <div className="publisher-launch-checklist__items">
                      {readinessTasks.map((task) => {
                        const Icon =
                          task.status === "done" ? CheckCircle2 : CircleDot;

                        return (
                          <div
                            className={`publisher-launch-task publisher-launch-task--${task.status}`}
                            key={task.id}
                            title={task.detail}
                          >
          ...
          accountPanel={
            <>
              <PublisherTabWorkflow
                ariaLabel={workflowLabels.account.label}
                steps={accountWorkflowSteps}
              />
              <section className="publisher-command-layout publisher-command-layout--single publisher-command-layout--account">
                <div className="publisher-command-main">
                  <div className="publisher-anchor-shell">
                    <PublisherAccountManager
                      account={publisherAccount}
                      locale={locale}
                      returnUrl={publisherReturnUrl}
                    />
                  </div>
                </div>
              </section>
          ...
          payoutPanel={
            <>
              <PublisherTabWorkflow
                ariaLabel={workflowLabels.payout.label}
                steps={payoutWorkflowSteps}
              />
              <section className="publisher-command-layout publisher-command-layout--payout publisher-payout-flow">
                <div className="publisher-command-main">
                  <article
                    className="ops-panel publisher-commercial-readiness"
                    id="publisher-paid-readiness"
                  >
                    ...
                    <div className="publisher-commercial-readiness__metrics">
                      {commercialMetrics.map(([label, value]) => (
                        <div
                          className="publisher-commercial-metric"
                          key={label}
                        >
                          <span>{label}</span>
                          <strong>{value}</strong>
                    ...
                    <div className="publisher-commercial-gates">
                      <div
                        className={
                          profileReady
                            ? "publisher-commercial-gate publisher-commercial-gate--ready"
                            : "publisher-commercial-gate"
                        }
                      >
                      ...
                      {commercialSkillRows.length > 0 ? (
                        <div className="publisher-commercial-table">
                          ...
                          {commercialSkillRows.map((row) => (
                            <div
                              className="publisher-commercial-row"
                              key={row.slug}
                            >
                              <strong>
                              ...
                              <span
                                className={
                                  row.ready
                                    ? "quality-chip quality-chip--complete"
                                    : "quality-chip quality-chip--attention"
                                }
                              >
                        ...
                        <div className="publisher-commercial-empty">
                          {labels.commercial.empty}
                        </div>
                      )}
                    </div>
                  </article>
                    ...
                    <div className="publisher-revenue-mix">
                      ...
                      <div className="publisher-revenue-mix__items">
                        {sourceMixRows.map((source) => (
                          <div
                            className="publisher-revenue-mix__item"
                            key={source.id}
                          >
                            <div>
                              ...
                              <span>
                                ...
                                {labels.sourceTransactionLabel}
                              </span>
                            </div>
                            <div className="publisher-revenue-mix__amount">
                              <strong>{formatMoney(source.grossCents)}</strong>
                              <span>
                                {formatMoney(source.publisherShareCents)}{" "}
                                {labels.sourceShareLabel}
                              </span>
                            </div>
                            <div
                              className="publisher-revenue-mix__bar"
                              aria-hidden="true"
                            >
                              <span
                    ...
                    <div className="ledger-table">
                      ...
                      {ledgerRows.length > 0 ? (
                        ledgerRows.map((transaction) => (
                          <div
                            className="ledger-row ledger-row--publisher-revenue"
                            key={transaction.id}
                          >
                            ...
                            <span className="publisher-ledger-source">
                              ...
                              {transaction.sourceReference ? (
                                <code>{transaction.sourceReference}</code>
                              ) : null}
                            </span>
                  ...
                  <article
                    className="ops-panel work-table-panel"
                    id="publisher-adjustments"
                  >
                    <div className="card-kicker">
                    ...
                    <div className="work-table work-table--adjustments">
                      ...
                      {adjustmentRows.length > 0 ? (
                        adjustmentRows.map((adjustment) => {
                          const Icon =
                            adjustment.typeKey === "refund"
                              ? RotateCcw
                              : ShieldAlert;

                          return (
                            <div
                              className="work-table__row adjustment-row"
                              key={`${adjustment.type}-${adjustment.id}`}
                            >
          ...
          demandPanel={
            <>
              <PublisherTabWorkflow
                ariaLabel={workflowLabels.demand.label}
                steps={demandWorkflowSteps}
              />
              <section className="publisher-command-layout publisher-command-layout--demand publisher-demand-flow">
                <div className="publisher-command-main">
                  <header className="publisher-flow-section-heading">
                    <span className="publisher-flow-section-heading__index">
                      01
                    </span>
                    <strong>{workflowLabels.demand.requestsTitle}</strong>
                  </header>
                ...
                <aside className="publisher-command-side">
                  <header className="publisher-flow-section-heading">
                    ...
                    <strong>{workflowLabels.demand.inboxTitle}</strong>
                  </header>
                  ...
                  <header className="publisher-flow-section-heading">
                    ...
                    <strong>{workflowLabels.demand.preferencesTitle}</strong>
                  </header>
