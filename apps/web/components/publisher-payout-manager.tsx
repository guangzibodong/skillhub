...
const copy = {
  en: {
    ...
    availableHelp: "Matured balance that can be reserved for a finance-reviewed payout request.",
    ...
    cannotRequest: "Payout review is blocked until these items are resolved.",
    expectedReview: "This request will enter manual finance review because it is above the review threshold.",
    expectedRequested: "This payout request can be created and then moves through finance review and manual transfer.",
    failureReason: "Failure reason",
    fixBlocker: "Fix",
    latestPayout: "Latest payout request",
    manualMethod: "Method",
    minimum: "Minimum",
    ...
    request: "Request finance review",
    ...
    blockerLabels: {
      ...
      no_available_balance: "No balance has matured yet.",
    ...
    nextActions: {
      ...
      earn_or_wait_minimum: "Wait for more balance to mature or keep earning until the minimum is reached.",
      request_again_after_failure: "Balances are available again; retry after the account or transfer issue is fixed.",
      request_payout: "Request finance review for all eligible balances.",
  ...
  zh: {
    ...
    accountState: "账户状态",
    available: "可用余额",
    availableHelp: "已经成熟、可以被财务复核打款申请锁定的余额。",
    blocked: "锁定/阻断",
    blockedHelp: "已被打款复核、人工转账或财务阻断占用的余额。",
    blockers: "阻断项",
    canRequest: "可进入财务复核",
    cannotRequest: "打款复核被阻断，请先处理下面这些事项。",
    expectedReview: "本次金额超过人工审核阈值，申请后会进入财务审核。",
    expectedRequested: "本次可以创建打款复核申请，然后进入财务审核和人工转账流程。",
    failureReason: "失败原因",
    fixBlocker: "去处理",
    latestPayout: "最近打款申请",
    manualMethod: "收款方式",
    ...
    blockerLabels: {
      amount_below_minimum: "可用余额还没有达到最低打款金额。",
      no_available_balance: "暂时没有已经成熟的余额。",
      ...
      publisher_payout_not_verified: "打款准备状态正在等待财务核验。",
    ...
    nextActions: {
      ...
      earn_or_wait_minimum: "继续等待余额成熟，或让余额达到最低打款金额。",
      request_again_after_failure: "余额已释放，收款账户或转账问题修复后可以重新申请。",
      request_payout: "申请财务复核，系统会锁定所有符合条件的可用余额。",
      resolve_blocker_before_retry: "先解决财务阻断原因，再重新申请提现。",
      wait_for_balance_maturity: "等待待成熟余额转为可用余额。"
    }
  }
} as const;
