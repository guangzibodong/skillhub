...
const copy = {
  en: {
    ...
    repairAction: "Fix action",
    repairCount: "{count} checks",
    repairDetail: "Current evidence",
    repairSection: "Section",
    repairTarget: "Open this check",
    repairTitle: "Launch repair workbench",
    title: "Launch readiness",
    targetPrefix: "Target",
    fallbackTarget: "Admin readiness",
    unavailableTarget:
      "Target module unavailable for this admin session; showing launch readiness instead.",
    sectionTitles: {
...
export function AdminLaunchReadinessPanel({
  ...
  locale,
  readiness,
}: AdminLaunchReadinessPanelProps) {
  const labels = copy[locale];
  const availablePanelIdSet = availablePanelIds
    ? new Set(availablePanelIds)
    : null;
  const visibleSections = readiness.sections.filter(
    (section) => section.items.length > 0,
  );
  const credibilityItems =
    visibleSections.find((section) => section.key === "launch_credibility")
      ?.items ?? [];
  const allPriorityItems = visibleSections
    .flatMap((section) =>
      section.items.map((item) => ({
        ...
        sectionTitle: sectionTitleFor(section.key, section.title, labels),
      })),
    )
    .filter(({ item }) => item.status !== "ready")
    .sort(
      (a, b) => priorityOrder[a.item.status] - priorityOrder[b.item.status],
    );
  const priorityItems = allPriorityItems.slice(0, 3);
  const repairItems = visibleSections.flatMap((section) =>
    section.items.map((item) => ({
      item,
      sectionKey: section.key,
      sectionTitle: sectionTitleFor(section.key, section.title, labels),
  ...
  return (
    <article className="ops-panel launch-readiness-panel">
      <header className="launch-readiness-head">
        <div>
          ...
          <div className="launch-readiness-env">
            <span>
              {labels.environment}: {readiness.environment.runtime}
            </span>
        ...
        <span className={statusClass(readiness.summary.status)}>
          {labels.statusLabels[readiness.summary.status]}
        </span>
      </header>

      <div className="launch-readiness-summary">
        <SummaryItem
          label={labels.summaryLabels.blocker}
          status="blocker"
          value={readiness.summary.blocker}
        />
        <SummaryItem
          label={labels.summaryLabels.warning}
          status="warning"
          value={readiness.summary.warning}
        />
        <SummaryItem
          label={labels.summaryLabels.ready}
          status="ready"
          value={readiness.summary.ready}
        />
        <SummaryItem
          label={labels.summaryLabels.deferred}
          status="deferred"
          value={readiness.summary.deferred}
        />
      </div>

      {credibilityItems.length > 0 ? (
        <section
          className="launch-readiness-proof"
          aria-label={labels.proofTitle}
        >
          <div className="launch-readiness-proof__head">
            ...
            <a
              className="launch-readiness-target-link"
          ...
          <div className="launch-readiness-proof__grid">
            {credibilityItems.map((item) => {
              ...
              const target = evidenceTargetFor(
                item,
                "launch_credibility",
                locale,
                labels,
              ...
              return (
                <a
                  className={`launch-readiness-proof-card launch-readiness-proof-card--${item.status}`}
                  href={target.href}
                  key={item.key}
                >
      ...
      <section
        className="launch-readiness-priority"
        aria-label={labels.priorityTitle}
      >
        <div className="launch-readiness-priority__head">
          <strong>{labels.priorityTitle}</strong>
          <span>
            {allPriorityItems.length > 0
              ? labels.priorityCount.replace(
                  "{count}",
                  String(allPriorityItems.length),
                )
        ...
        {priorityItems.length > 0 ? (
          <div className="launch-readiness-priority__list">
            {priorityItems.map(({ item, sectionKey, sectionTitle }) => {
              ...
              return (
                <div
                  className={`launch-readiness-priority__item launch-readiness-priority__item--${item.status}`}
      ...
      {repairItems.length > 0 ? (
        <section
          className="launch-readiness-repair"
          aria-label={labels.repairTitle}
        >
          <div className="launch-readiness-repair__head">
            <strong>{labels.repairTitle}</strong>
            <span>
              {labels.repairCount.replace(
                "{count}",
                String(repairItems.length),
              )}
            </span>
          </div>
          <div className="launch-readiness-repair__grid">
            {repairItems.map(({ item, sectionTitle }) => (
              <article
                className={`launch-readiness-repair-card launch-readiness-repair-card--${item.status}`}
                id={repairAnchorId(item.key)}
                key={item.key}
              >
                <header>
                  <div>
                    <span>
                      {labels.repairSection}: {sectionTitle}
                    </span>
                    <strong>{item.label}</strong>
                  </div>
                  <span className={statusClass(item.status)}>
                ...
                <dl>
                  <div>
                    <dt>{labels.repairDetail}</dt>
                    <dd>{item.detail}</dd>
                  </div>
                  <div>
                    <dt>{labels.repairAction}</dt>
                    <dd>{item.action}</dd>
                  </div>
                </dl>
                <a
                  className="launch-readiness-target-link"
                  href={`#${readinessItemAnchorId(item.key)}`}
                >
                  <span>{labels.repairTarget}</span>
                  <ArrowRight size={14} aria-hidden="true" />
                </a>
              </article>
            ))}
          ...
          {visibleSections.map((section) => (
            <section className="launch-readiness-section" key={section.key}>
              <div className="launch-readiness-section__head">
                <strong>
                  {sectionTitleFor(section.key, section.title, labels)}
                </strong>
                <span className={statusClass(section.status)}>
                  {labels.statusLabels[section.status]}
                </span>
              </div>
              ...
              <div className="launch-readiness-items">
                {section.items.map((item) => (
                  <ReadinessItem
                    availablePanelIds={availablePanelIdSet}
                    item={item}
                    key={item.key}
                    labels={labels}
                    locale={locale}
...
function SummaryItem({
  ...
  return (
    <div
      className={`launch-readiness-summary__item launch-readiness-summary__item--${status}`}
    >
      <Icon size={16} aria-hidden="true" />
      <span>{label}</span>
      <strong>{value}</strong>
...
function ReadinessItem({
  ...
  availablePanelIds: ReadonlySet<string> | null;
  ...
  return (
    <div
      className={`launch-readiness-item launch-readiness-item--${item.status}`}
      id={readinessItemAnchorId(item.key)}
    >
      <Icon size={16} aria-hidden="true" />
      <div>
        ...
        <div className="launch-readiness-item__target">
          <a className="launch-readiness-target-link" href={target.href}>
            <span>
              {item.status === "ready" ? labels.evidenceCta : labels.fixCta}
            </span>
            <ArrowRight size={14} aria-hidden="true" />
          </a>
          <small>
            {labels.targetPrefix}: {target.scope}
          </small>
        </div>
      </div>
    </div>
...
function sectionTitleFor(
  ...
  return (
    labels.sectionTitles[sectionKey as keyof typeof labels.sectionTitles] ??
    fallback
  );
}
...
function evidenceTargetFor(
  ...
  const target = readinessTargets[item.key] ??
    sectionTargets[sectionKey] ?? {
  ...
  const targetPanelId = adminPanelIdFromHref(target.href);

  if (
    targetPanelId &&
    availablePanelIds &&
    !availablePanelIds.has(targetPanelId)
  ) {
    return {
      href: localizedHref("/admin#launch-readiness", locale),
      scope: `${target.scope[locale]} - ${labels.unavailableTarget}`,
    };
  }
...
function adminPanelIdFromHref(href: string) {
  const [path, hash] = href.split("#");

  return path === "/admin" && hash ? hash : null;
}

function repairHrefFor(key: string) {
  return `#${repairAnchorId(key)}`;
}

function repairAnchorId(key: string) {
  return `readiness-repair-${key}`;
}

function readinessItemAnchorId(key: string) {
  return `readiness-item-${key}`;
}

const readinessTargets: Record<
  string,
  { href: string; scope: Record<Locale, string> }
> = {
  ...
  email_auth_secret: {
    href: repairHrefFor("email_auth_secret"),
    scope: {
  ...
  email_challenge_storage: {
    href: repairHrefFor("email_challenge_storage"),
    scope: {
  ...
  password_credential_storage: {
    href: repairHrefFor("password_credential_storage"),
    scope: {
      en: "Password credential migration state",
      zh: "\u5bc6\u7801\u51ed\u636e\u8fc1\u79fb\u72b6\u6001",
    },
  },
  email_debug_codes: {
    href: repairHrefFor("email_debug_codes"),
    scope: {
  ...
  email_provider: {
    href: repairHrefFor("email_provider"),
    scope: {
  ...
  github_oauth: {
    href: repairHrefFor("github_oauth"),
    scope: {
  ...
  google_oauth: {
    href: repairHrefFor("google_oauth"),
    scope: {
  ...
  legacy_signup: {
    href: repairHrefFor("legacy_signup"),
    scope: {
  ...
  notification_delivery_schema: {
    href: repairHrefFor("notification_delivery_schema"),
    scope: {
  ...
  oauth_callback_base_url: {
    href: repairHrefFor("oauth_callback_base_url"),
    scope: {
  ...
  oauth_providers: {
    href: repairHrefFor("oauth_providers"),
    scope: {
  ...
  oauth_state_secret: {
    href: repairHrefFor("oauth_state_secret"),
    scope: {
  ...
  public_signup_policy: {
    href: repairHrefFor("public_signup_policy"),
    scope: {
  ...
  session_cookie_domain: {
    href: repairHrefFor("session_cookie_domain"),
    scope: {
