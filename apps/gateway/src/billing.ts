...
export async function getFinanceLedger() {
  ...
  const [summaryRows, recentTransactions, unprocessedRows, unprocessedSubscriptionRows, renewableSubscriptionRows] = await Promise.all([
    sql`
      select
        ...
        coalesce(sum(t.amount_cents) filter (where t.source_type = 'usage'), 0)::int as usage_gross_cents,
        coalesce(sum(ts.platform_fee_cents) filter (where t.source_type = 'usage'), 0)::int as usage_platform_fee_cents,
        coalesce(sum(ts.publisher_share_cents) filter (where t.source_type = 'usage'), 0)::int as usage_publisher_share_cents,
        (count(distinct t.id) filter (where t.source_type = 'usage'))::int as usage_transaction_count,
        coalesce(sum(t.amount_cents) filter (where t.source_type = 'subscription'), 0)::int as subscription_gross_cents,
        coalesce(sum(ts.platform_fee_cents) filter (where t.source_type = 'subscription'), 0)::int as subscription_platform_fee_cents,
        coalesce(sum(ts.publisher_share_cents) filter (where t.source_type = 'subscription'), 0)::int as subscription_publisher_share_cents,
        (count(distinct t.id) filter (where t.source_type = 'subscription'))::int as subscription_transaction_count,
  ...
  return {
    summary: {
      ...
      usageGrossCents: Number(summary.usage_gross_cents ?? 0),
      usagePlatformFeeCents: Number(summary.usage_platform_fee_cents ?? 0),
      usagePublisherShareCents: Number(summary.usage_publisher_share_cents ?? 0),
      usageTransactionCount: Number(summary.usage_transaction_count ?? 0),
      subscriptionGrossCents: Number(summary.subscription_gross_cents ?? 0),
      subscriptionPlatformFeeCents: Number(summary.subscription_platform_fee_cents ?? 0),
      subscriptionPublisherShareCents: Number(summary.subscription_publisher_share_cents ?? 0),
      subscriptionTransactionCount: Number(summary.subscription_transaction_count ?? 0),
  ...
  const [summaryRows, recentTransactions, unprocessedRows, unprocessedSubscriptionRows, renewableSubscriptionRows] = await Promise.all([
    sql`
      select
        ...
        coalesce(sum(ts.platform_fee_cents), 0)::int as platform_fee_cents,
        coalesce(sum(ts.publisher_share_cents), 0)::int as publisher_share_cents,
        coalesce(sum(t.amount_cents) filter (where t.source_type = 'usage' and t.amount_cents > 0 and t.status = 'posted'), 0)::int as usage_gross_cents,
        coalesce(sum(ts.platform_fee_cents) filter (where t.source_type = 'usage' and t.amount_cents > 0 and t.status = 'posted'), 0)::int as usage_platform_fee_cents,
        coalesce(sum(ts.publisher_share_cents) filter (where t.source_type = 'usage' and t.amount_cents > 0 and t.status = 'posted'), 0)::int as usage_publisher_share_cents,
        (count(distinct t.id) filter (where t.source_type = 'usage' and t.amount_cents > 0 and t.status = 'posted'))::int as usage_transaction_count,
        coalesce(sum(t.amount_cents) filter (where t.source_type = 'subscription' and t.amount_cents > 0 and t.status = 'posted'), 0)::int as subscription_gross_cents,
        coalesce(sum(ts.platform_fee_cents) filter (where t.source_type = 'subscription' and t.amount_cents > 0 and t.status = 'posted'), 0)::int as subscription_platform_fee_cents,
        coalesce(sum(ts.publisher_share_cents) filter (where t.source_type = 'subscription' and t.amount_cents > 0 and t.status = 'posted'), 0)::int as subscription_publisher_share_cents,
        (count(distinct t.id) filter (where t.source_type = 'subscription' and t.amount_cents > 0 and t.status = 'posted'))::int as subscription_transaction_count,
        coalesce(sum(pb.amount_cents) filter (where pb.state = 'pending'), 0)::int as pending_balance_cents,
        coalesce(sum(pb.amount_cents) filter (where pb.state = 'available'), 0)::int as available_balance_cents
