...
- Notification processing must apply user channel preferences and organization webhook subscriptions by fanning out in-app business events into email and webhook delivery queues, then rendering active templates at delivery time so operators can change communication copy without code deploys.
