...
Product and design work must follow [Product Management Operating Model](./product-management-operating-model.md). No new UI or implementation work should begin without a feature requirement using [Feature Requirement Template](./feature-requirement-template.md), unless the change is a small verified bug fix. Open launch and policy decisions are tracked in [Product Decision Log](./product-decision-log.md), the current P0 freeze is recorded in [Requirements Freeze Workshop](./requirements-freeze-workshop.md), and the approved UI-entry journeys are defined in [P0 Journey Specs](./p0-journey-specs.md).
...
- Fresh production deployments must seed active default notification templates for account access, skill review, runtime incidents, billing, payouts, buyer requests, feedback, trust reports, marketplace curation, and delivery operations, with English and Chinese variants where user-facing, without overwriting templates already edited by operators.
- Notification processing must apply user email channel preferences and organization webhook endpoint subscriptions by fanning out in-app business events into email and webhook delivery queues, then rendering active templates at delivery time so operators can change communication copy without code deploys. Organization webhook delivery is controlled by endpoint `status` and subscribed `events`; individual user webhook preferences must not suppress organization-level webhook fanout.
...
- Launch readiness command panel that checks production configuration and operating prerequisites for OAuth callbacks, cookie domain, email-code delivery, webhook worker schema, database migrations, required active notification-template coverage, runtime API-key hashing, commission rules, publisher terms acceptance, payout state, demo fallback, legacy signup, service token, and public signup policy.
- Launch readiness must also check launch credibility thresholds for verified public skills, active publishers with public supply, active developer projects, successful governed invocations, and published buyer feedback.
...
- An active subscription can advance to the next monthly period only after the previous period has a posted subscription transaction; this prevents runtime renewal from skipping revenue recognition.
