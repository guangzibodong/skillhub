...
- Launch-readiness QA now has a shared contract validator used by both the general smoke and `pnpm smoke:p0:admin`. The validator requires the identity, email, webhook, marketplace-operations, launch-credibility, commercial, and production-guardrail sections plus their customer-demo evidence item keys, item fields, valid statuses, and matching summary counts, while still allowing real environments to report blockers or warnings.
