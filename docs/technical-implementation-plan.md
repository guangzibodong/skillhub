...
- `pnpm smoke:p0:demo` now adds a mutating end-to-end customer-demo smoke that creates a generated publisher draft, submits and approves the exact version, verifies public marketplace discovery, creates a developer project, saves and installs the verified skill, creates a reveal-once project key, runs governed console runtime and MCP `tools/call` invocations, then verifies project detail, notification, admin audit, and admin notification handoff without direct database access or credential exposure.
