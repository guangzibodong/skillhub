...
- Public registry reads now fail soft only for incomplete registry-read schema in production-like deployments: `/v1/skills/search`, public manifest reads, and public MCP discovery return empty marketplace-safe results instead of demo rows or 500s when tables such as `skill_reviews` are missing. Internal operational probes, writes, admin queues, and launch readiness still surface migration blockers. The general smoke now checks `/v1/skills/search?limit=5`, fails when public stats report published supply but search returns none, and checks a real returned `/skills/:slug` detail page instead of assuming a production demo slug exists.
