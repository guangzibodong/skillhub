...
- The general production smoke now checks the unauthenticated boundary for critical Journey C read endpoints, including review, finance ledger, payouts, notifications, delivery queues, webhook deliveries, identity directory, audit logs, and marketplace curation. This keeps the routine public no-write gate able to catch accidental admin-data exposure even when the full protected Journey C token is not configured.
