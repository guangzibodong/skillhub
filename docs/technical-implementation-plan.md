...
- Published feedback is public through a buyer-safe projection only; reviewer identity, project slugs, moderation reasons, operator notes, audit metadata, and credentials stay out of the public payload. New feedback starts in moderation, and admin decisions are audited.
...
- P0 payout setup uses publisher-submitted PayPal or Alipay receiving details. The gateway accepts only `manual_deferred`, validates the manual method/account payload, and stores a setup session for finance verification.
- Payout requests above a threshold enter manual review.
- Blocked payout records reason and retry condition.
- Publisher payout summary exposes readiness blockers, request eligibility, manual receiving-account details, expected initial payout status, balance-state explanations, latest payout reason, retry condition, and next action.
- Finance payout decisions require a manual transfer reference when marking paid and retry condition when blocking a payout.
...
- Admin payout review console now reads the live payout queue and exposes approve, mark-paid, fail, and block decisions with finance notes, PayPal/Alipay receiving details, transfer references, retry conditions, and next-action state.
- Payout decision enforcement now rejects `mark_paid` without a transfer reference and records the acting finance operator in the payout audit row when available.
...
- Username/email password access is modeled through migration `033_password_credentials.sql`, `/v1/auth/password/signup`, and `/v1/auth/password/login`, with per-user PBKDF2-SHA256 salted hashes, no raw password persistence, 14-day user session tokens, audit logs, and httpOnly web-session storage.
...
- `/login` now behaves like a product account entry instead of a token-only console: username/email password signup/login is active, email-code access remains available through API flows, Google/GitHub login paths become real provider redirects when configured, and token login remains the operator/team fallback.
...
- `/` and `/account` now expose a role-aware console access map for `/login`, `/account`, `/developer`, `/publisher`, and `/admin`, making backend routes, required roles, no-shared-password access, OAuth configuration state, and token fallback boundaries visible before customers need private instructions. The public `/login` page no longer advertises the admin shortcut.
...
- Manual payout setup session records for PayPal/Alipay receiving-detail handoff, finance completion or blocking, audit, and notifications.
- Publisher dashboard reads payout account and onboarding state.
- Dashboard publisher account panel now lets publishers update their public profile name and submit PayPal/Alipay receiving details. Finance/admin operators, not publishers, record verification or blocking decisions while preserving existing restricted/suspended publisher status when no status change is requested.
...
- Public feedback APIs now expose published feedback plus full published-rating summary for each skill through a marketplace-safe contract, while user-scoped submissions enter moderation. Public rows keep rating, title, body, use case, publish timestamps, and publisher responses, but exclude reviewer email/display name, reviewer organization, project slug, moderation reason, moderator metadata, audit fields, tokens, credentials, and operator notes.
...
- Skill detail pages now show published user feedback, average rating, public use-case context, publisher responses, and a signed-in feedback submission form before the trust report flow. Private reviewer/project/moderation fields remain available only to publisher/admin workflows.
...
- Marketplace discovery now supports URL-initialized and URL-preserved search/filter state for query, category, pricing, permission risk, runtime, verification, and sort. The marketplace server render now forwards recognized URL filters into `/v1/skills/search`, including the public category contract, so homepage handoff and shared discovery links fetch the same catalog slice before client-side refinement starts.
...
- `/login` now opens with a normal account entry: Google/GitHub provider buttons, username/email password login, registration with username/email/password/workspace details, and secondary token recovery. It keeps OAuth buttons disabled unless provider configuration is actually ready.
...
- The global sign-in action now lands on `/login`, `/admin-login` redirects to the normal login page without exposing an admin shortcut, and the non-mutating app smoke asserts P0 operating markers plus Chinese mojibake guards across home, login, dashboard, publish, publisher, developer, and admin surfaces. Dedicated workspaces remain `/developer`, `/publisher`, and `/admin`.
- The routine public P0 smoke now also includes a production web alias gate when targeting `https://useskillhub.com`. It checks `www.useskillhub.com` and `app.useskillhub.com` for the home page and Chinese login entry, accepts same-site redirects, and fails on non-SkillHub hosts, stale HTML, missing P0 markers, or mojibake so DNS/reverse-proxy drift is caught during 1Panel updates.
...
- The Journey A and Journey B mutating smoke scripts now reuse the shared sensitive-output validator for protected API responses. Developer project detail, developer notifications, admin audit rows, publisher workspace reads, review queues, and notification queues fail if they expose authorization-shaped strings, raw user/project/webhook/provider keys, raw API-key fields, or email-code previews; the only permitted raw project key remains the reveal-once creation response that the script does not re-scan as a leak.
...
- The P0 admin operations smoke now applies the shared sensitive-output validator to protected overview, finance ledger, commission-rule, refund, dispute, and payout reads as well as readiness, review, notification, webhook, identity, audit, trust, and curation reads. This keeps Journey C money-state inspection inside the same secret-safe QA gate as launch readiness and review evidence.
- The P0 admin operations smoke now also validates protected notification-template reads as a first-class Journey C operation. `GET /v1/admin/notification-templates` must require admin authorization, return rows with template identity, channel, locale, subject, body, status, and update time, and pass the same sensitive-output scan as delivery, audit, and readiness reads.
- `pnpm smoke:p0` now provides a fixed-order P0 release gate. By default it runs the public production-safe smoke and the non-mutating Journey C admin operations smoke, while Journey A, Journey B, and full demo-chain writes remain explicit opt-ins with the child scripts' existing production-write guards.
- The 1Panel update and deployment runbooks now use that release gate directly: routine updates run `pnpm smoke:p0 -- --prod --skip-admin --timeout-ms 30000` as the public no-write gate, while full protected Journey C signoff runs `pnpm smoke:p0 -- --prod --timeout-ms 30000` only when an admin/super-admin user token is already configured in the shell.
- The routine public P0 smoke now starts with a production API health gate. For `https://api.useskillhub.com`, `/health` must return the SkillHub gateway payload with `env=production`, so wrong reverse proxies, stale API containers, or non-production boot modes fail before deeper endpoint checks run.
- The P0 release suite help and API reference now show the same two production modes, and the general smoke statically guards `scripts/qa-p0-release-suite.mjs`, `docs/api.md`, and the 1Panel/QA runbooks so the routine public no-write gate and full protected Journey C gate cannot quietly drift apart.
...
- `pnpm smoke:p0:demo` now adds a mutating end-to-end customer-demo smoke that creates a generated publisher draft, submits and approves the exact version, verifies public marketplace discovery, submits manual PayPal payout details, verifies payout readiness through a finance/admin operator path, activates a per-call price, creates a developer project, saves and installs the verified skill, creates a reveal-once project key, runs governed console runtime and billable MCP `tools/call` invocations, processes billable usage into posted transaction/split/publisher-balance rows, releases eligible balances, requests publisher payout, approves it, records the manual transfer reference, then verifies project detail, publisher/admin finance and payout ledgers, notification, admin audit, and admin notification handoff without direct database access or credential exposure.
- The P0 demo-chain smoke now also imports the shared sensitive-output validator used by the Journey A/B scripts. Project detail, publisher/developer notifications, finance and payout ledgers, admin audit rows, and admin notification queues fail if they expose authorization-shaped strings, raw user/project/webhook/provider keys, raw API-key fields, or email-code previews; the reveal-once project API-key creation response remains the only intentional raw key handoff.
...
- The general smoke now also has a deployment-runbook P0 gate guard. Before API or app requests, it verifies that the 1Panel update/deploy docs and QA guide keep the routine post-update command on `pnpm smoke:p0 -- --prod --skip-admin --timeout-ms 30000`, keep the protected Journey C gate separate for preconfigured admin/super-admin user tokens, and do not regress to the old service-token-shaped `smoke:prod` update path.
- The general production smoke now checks the unauthenticated boundary for critical Journey C read endpoints, including review, finance ledger, payouts, notifications, notification templates, delivery queues, webhook deliveries, identity directory, audit logs, and marketplace curation. This keeps the routine public no-write gate able to catch accidental admin-data exposure even when the full protected Journey C token is not configured.
- The general production smoke now also checks unauthenticated organization, developer, and publisher workspace read boundaries for team, webhooks, billing, developer projects, publisher skills, publisher payouts, publisher finance ledger, and publisher marketplace appeals. These no-write checks require JSON `401` or `403` responses and run the sensitive-output scanner so routine 1Panel updates can catch accidental Journey A/B workspace exposure without needing an operator token.
- The general production smoke now extends the same no-token boundary check to representative write endpoints across publishing, feedback, trust reports, subscriptions, team management, webhook setup and secret rotation, billing profile, developer projects, buyer requests, project saves/installs/policies/API keys/runtime tests, direct runtime invocation, pricing, publisher versions, marketplace appeals, publisher feedback responses, publisher profile, terms, payout onboarding, and payout requests. Each request must stop at JSON `401` or `403` with no sensitive-output leak, keeping the routine gate no-write while protecting Journey A/B/C mutation surfaces.
- The general production smoke now covers representative Journey C admin write boundaries without credentials as well: review decisions, commission-rule creation, finance processors, refunds, disputes, payout decisions, notification and webhook delivery operations, notification templates, marketplace curation, curation appeals, incidents, abuse-report decisions, and feedback moderation. These probes still create no rows because they intentionally omit tokens, but they catch admin mutation routes that accidentally become public.
- Manual payout setup now rejects unsupported provider names, validates PayPal/Alipay receiving-account details, rejects URL-shaped account payloads, and launch readiness treats the manual payout columns plus setup sessions as required payout-state schema before paid marketplace rollout. The P0 demo-chain smoke also asserts manual method/account persistence during commercial-readiness setup.
...
- The general production smoke now also guards the public marketplace list contract itself. When `/v1/skills/search` returns skills, each row must include the buyer-safe operating signals that Journey A cards need: identity, version, verification state, permission risk, runtime type, billing model, install/call counts, success, latency, rating, and feedback. The same check fails if internal curation fields such as boost or operator reasons leak into public discovery.
- The general production smoke now validates the public detail API contracts behind those pages as well. When public search returns a real skill, it checks `/v1/skills/:slug` for manifest identity, runtime, permission, and schema fields; when the publisher directory returns a real publisher, it checks `/v1/publishers/:slug` for trust level, payout status, public metrics, and public skill rows before rendering profile pages.
- The general smoke now validates skill-detail support APIs as part of the same Journey A public contract. When a real public skill exists, it checks `/v1/skills/:slug/feedback` for published-feedback summary shape and reviewer/project/moderation field exclusion, and checks `/v1/skills/:slug/prices` for active public price-row shape plus provider/project/ledger/commission field exclusion, so feedback and subscription/price UI on the detail page has live API backing without leaking Journey B/C internals.
- The general smoke now also runs a source-level mojibake guard before any API or app checks. It scans the web app source, QA scripts, and docs for grouped historical encoding-corruption markers such as replacement characters, the U+951F/U+65A4/U+62F7 replacement sequence, and UTF-8-as-GBK fragments, so bilingual copy regressions in files like the publish entry are caught even before they reach rendered HTML.
- The general smoke now also guards dynamic public skill detail controls. When a real public skill exists, both `/skills/:slug` and `/skills/:slug?lang=zh` must render the developer handoff packet, feedback submission entry, and trust report entry, and the mojibake scanner includes common UTF-8-as-GBK fragments from Chinese Journey A copy so detail pages cannot silently degrade into a 200 page without install, feedback, or trust actions.
- The general smoke now also checks unauthenticated Journey A/B write boundaries for skill detail actions, project actions, publisher actions, organization operations, runtime invocation, pricing, payout onboarding, and payout requests. The routine public gate sends write-shaped requests without credentials and requires `401` or `403` JSON errors with no sensitive-output leaks, proving that visible workspace controls are not accidentally writable during production updates.
- Runtime invocation summaries are now secret-safe before they are stored. `input_summary` and `output_summary` redact sensitive key names and authorization-shaped strings, limit deep or long values, and the disabled-proxy metered output no longer echoes the raw invocation input back into the persisted output summary.
- Public skill detail now returns a sanitized manifest contract instead of the raw publisher manifest. `/v1/skills/:slug` preserves identity, version, tags, runtime type, permissions, and schemas for Journey A inspection, while redacting embedded URL credentials, local command details, and secret handles; the general smoke enforces that no project, operator, curation, invocation, billing, payout, audit, raw secret-handle, or embedded-credential fields leak through the public detail endpoint.
- Public MCP discovery now exposes the same marketplace-safe contract shape as public skill detail for agent clients: `tools/list` includes identity, input/output schemas, tags, semantic version, runtime type, and permission risk, while omitting project install, approval, callable, curation, and operator fields. The general routine smoke verifies that public search supply is discoverable through MCP and that unauthenticated `tools/call` returns a non-mutating MCP `isError` boundary with no invocation, billable, or runtime output state.
- Public MCP resource reads now return a sanitized public contract copy instead of the raw stored manifest. `resources/read` keeps identity, version, author, tags, schemas, runtime type/target, permission risk, and secret count, while removing embedded URL credentials, local command details, secret handles, project state, curation fields, operator notes, invocation records, and billing state. The general smoke now exercises `resources/list` and `resources/read` alongside `tools/list` so agent discovery regressions are caught during the no-write production gate.
- Public publisher API QA now treats `/v1/publishers` and `/v1/publishers/:slug` as marketplace-safe supplier trust contracts. The general smoke fails if those public payloads expose private organization ids, members, billing/payment records, payout accounts or onboarding sessions, provider references, internal curation boost/reason, operator notes, audit rows, webhook secrets, raw tokens, API keys, credentials, or embedded URL credentials, while still allowing coarse public readiness signals such as `payoutStatus`.
