...
- The general smoke now also checks unauthenticated Journey A write boundaries for skill feedback, skill abuse reports, and project subscription creation. The routine public gate sends POST requests without credentials and requires `401` or `403` JSON errors with no sensitive-output leaks, proving that the skill detail action entry points and subscription trial path are not accidentally writable during production updates.
