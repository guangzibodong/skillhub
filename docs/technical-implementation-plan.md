...
- External delivery processing fans in-app business events into email/webhook queue rows using user notification preferences and organization webhook endpoint subscriptions, then renders the active template for the delivery channel and locale at send time.
...
- `/v1/admin/notification-deliveries/process` now first fans out eligible in-app business notifications into external email/webhook delivery rows according to user preferences and organization webhook subscriptions, reports fanout counts, and renders active templates for email text and webhook JSON payloads at delivery time.
