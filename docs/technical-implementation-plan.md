...
- The global sign-in action now lands on `/login`, `/admin-login` redirects to the login page's admin-entry section, and the non-mutating app smoke asserts P0 operating markers plus Chinese mojibake guards across home, login, dashboard, publish, publisher, developer, and admin surfaces. Dedicated workspaces remain `/developer`, `/publisher`, and `/admin`.
