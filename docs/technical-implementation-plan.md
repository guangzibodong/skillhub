...
- The general smoke now validates skill-detail support APIs as part of the same Journey A public contract. When a real public skill exists, it checks `/v1/skills/:slug/feedback` for published-feedback summary shape and `/v1/skills/:slug/prices` for public price-row shape, so feedback and subscription/price UI on the detail page has live API backing even when no production rows exist yet.
