...
- Uses grouped admin navigation and a responsive console layout: desktop keeps persistent grouped navigation, while tablet/mobile collapse into horizontally scrollable groups, single-column KPI cards, and contained table scrolling without page-level horizontal overflow.
- Shows an order/payment command area from the finance ledger so operators can inspect recent order-like ledger references, payment/refund/dispute state, and provider-gated Stripe/Alipay status without implying live payment capture before integration.
- Shows a traffic and IP access analysis area for UV, page views, unique IPs, suspicious IPs, referrers, and hot paths. Until access logs, Cloudflare analytics, or an equivalent analytics source is connected, this area must be clearly source-gated and must not show estimated or fake traffic numbers.
- Cloudflare GraphQL Analytics may provide aggregate request, visitor, and country/region distribution through `CLOUDFLARE_ZONE_ID` and `CLOUDFLARE_API_TOKEN`; raw recent IP inspection must come from mounted 1Panel/Nginx access logs rather than Cloudflare aggregate analytics.
