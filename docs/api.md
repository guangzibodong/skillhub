...
The public marketplace and skill detail pages read these registry endpoints first. The response is a public contract copy, not the raw publisher manifest: HTTP/MCP runtime URLs remove embedded credentials, local runtimes expose only a restricted placeholder plus redacted arg count, author URLs remove embedded credentials, and secret handles are reduced to a count placeholder while preserving high-risk permission state. The routine public smoke fails if `/v1/skills/:slug` exposes project, operator, curation, invocation, billing, payout, audit, raw secret-handle, local-command, or embedded-credential details.

In local development or controlled staging demos, bundled demo content can fill unavailable registry responses. In production-like runtimes (`NODE_ENV=production`, `VERCEL_ENV=production`, or `SKILLHUB_ENV=production`), demo fallback is disabled unless `SKILLHUB_ENABLE_DEMO_FALLBACK=true` is explicitly set, so unavailable or empty live registry data renders an empty marketplace or a not-found skill detail instead of fake supply. Skill cards merge search summaries, public manifest runtime/permission data, and public price records from `/v1/skills/:slug/prices`. The marketplace uses `/v1/skills/search?sort=recommended` as the first discovery source before local interactive filtering.
...
Passing assertions mean the admin console can read launch readiness, preserve the required readiness section/item contract, review evidence, finance ledger state, payout explainability, notifications, notification templates, webhook outbox, identity directory, audit logs, trust queues, and marketplace curation queues without writing data or exposing authorization-shaped secrets. The smoke scans protected overview, finance ledger, commission-rule, refund, dispute, payout, and notification-template reads with the same sensitive-output validator used for launch readiness, review evidence, notifications, identity, and audit logs.
