...
The same no-token smoke also checks protected organization, developer, publisher, and admin read endpoints plus representative workspace and admin write endpoints. These checks perform no writes because they intentionally omit credentials; they require JSON `401` or `403` responses and scan the boundary body for authorization-shaped strings, raw keys, email-code previews, webhook secrets, and provider credentials. Admin write coverage includes review decisions, commission and ledger processors, refunds, disputes, payout decisions, notification/webhook delivery operations, notification templates, marketplace curation, incidents, abuse-report decisions, and feedback moderation.
...
- Finance release processing moves matured publisher balances to `available`; the publisher then requests payout, finance approves it, finance records a synthetic manual transfer reference, and publisher/admin payout reads must expose the paid state.
