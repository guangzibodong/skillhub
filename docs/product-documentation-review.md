...
Acceptance-team update on 2026-06-09: role-based launch QA now has a safe server-side initializer and runbook. `pnpm acceptance:team` creates a normal developer account, a normal publisher partner account, and a server-promoted admin/operator account, verifies password login plus role access, stores generated credentials only in a private local file, and maps the three identities to Journey A, Journey B, and Journey C walkthroughs in `docs/acceptance-team-qa.md`.

Acceptance-team collector update on 2026-06-10: `pnpm qa:acceptance-team` now reads the private acceptance-team credential file on the server, logs in as developer, publisher, and admin/operator, checks role-scoped APIs and logged-in workspace pages with the real session cookie, and writes a secret-safe issue report. This separates "smoke passes" from "role workflows feel launch-ready" without asking operators to paste credentials into chat or reports.

Acceptance-team marker update on 2026-06-10: the role-based collector now treats active workspace copy and stable HTML anchors separately. Logged-in role pages are still P0 if they show locked-state copy, fail protected APIs, or lose required workspace sections, but launch-readiness blockers/warnings remain P1/P2 report items instead of being hidden or downgraded.

Launch QA update on 2026-06-10: the acceptance-team report now includes secret-safe launch-readiness item details for blockers, warnings, and deferred work, so operator QA can see the exact section, item, action, and evidence text instead of only summary counts. Mobile skill-detail QA also replaces first-viewport `n/a` success/latency metrics with localized preview-state labels while preserving the underlying source-of-truth values and gated runtime model.
...
The latest publisher signed-out polish makes `/publisher` less like a locked internal dashboard for first-time authors. Signed-out and role-gated visitors now see one workspace access panel plus a publisher access guide with sign-in, account-role confirmation, and first-skill upload actions. The redundant signed-out session card and operational-signal chips were removed from the locked state, while full publisher operations still remain visible only after publisher, owner, admin, or super-admin access is ready.
...
The latest account/project access polish further reduces signed-out console noise. `/account` now stops after the account-entry command and a three-step access guide instead of rendering the role map, empty profile, empty session, and notification panels for visitors without a session. `/dashboard/projects/[slug]` signed-out and role-gated states now use the same three-step access pattern as `/developer`: sign in, confirm developer access, then return to project operations for keys, policy, and governed runtime testing. Hidden P0 markers preserve smoke coverage without forcing operational chips into the visible locked state.
...
The latest shared workspace-session polish fixes a confusing signed-out state across `/publisher`, `/developer`, `/dashboard`, `/account`, and other protected surfaces. `SessionStatusPanel` now derives its heading from the actual browser session: signed-in users see the account-ready state, signed-out users see sign-in-required copy, and invalid/stale sessions show a recovery state instead of implying that the user is already logged in. The empty state now includes a direct sign-in action, and the signed-out card uses the same restrained warning treatment as role-gated workspace access. Locked developer, publisher, and admin journey rails now keep their lifecycle context but route the primary CTA to sign-in or account role review instead of sending signed-out users deeper into protected workflow anchors.

The latest publish-entry hierarchy pass moves the publisher access board and manifest form before the longer operating pipeline on `/publish`. Mobile users now see whether they can publish and, when allowed, can start from the manifest editor before reading the full review/pricing/payout explanation. The pipeline remains on the page as context below the primary action, preserving the same Journey B scope while reducing the chance that a signed-out publisher reads a long workflow before discovering the form is locked.

The latest account-center hierarchy pass makes `/account` read more like a personal center before it reads like an operator map. The account state strip now appears before the compact console-access map, so signed-out and mobile users first see identity, security, workspace, and operations status before the role-specific backend routes. Compact mobile console maps also suppress the longer no-shared-password/OAuth/token rule explanations, reducing first-screen density while preserving the same role-gated workspace cards and full rule copy on wider layouts.
...
The latest mobile-navigation polish keeps the compact header focused on public discovery without exposing admin login. The mobile menu button now has hidden text in addition to its icon and ARIA label, and the opened mobile menu includes Home, Registry, Marketplace, Docs, Publish, and Sign in. Account and operator paths stay behind the normal sign-in/account flow instead of appearing as anonymous navigation shortcuts.
...
The latest 1Panel deployment hardening captures the stale Docker image failure found during production verification. The update and deploy runbooks now require rebuilding the `api` and `web` compose services with `--no-cache`, then recreating the `skillhub-api` and `skillhub-web` containers with `--force-recreate`; the deploy script uses the same sequence. The routine public P0 smoke also guards those commands so future docs cannot drift back to a plain `docker restart` or `up -d --build` path that leaves production serving old gateway code.

The latest domain-alignment pass makes `useskillhub.com` the primary public web entry while keeping `www.useskillhub.com` and `app.useskillhub.com` as valid web aliases and `api.useskillhub.com` as the API origin. Production defaults, OAuth/email return URLs, CORS, smoke targets, and 1Panel runbooks now use the main domain first so customer links, login callbacks, public terms, registry docs, and release verification all point at the brand domain.

The latest production web alias gate turns that domain decision into release QA. The routine no-write P0 smoke now checks `www.useskillhub.com` and `app.useskillhub.com` whenever the app target is `https://useskillhub.com`, verifying the home page and Chinese login entry, accepting same-site redirects, and failing on non-SkillHub hosts, stale HTML, missing P0 markers, or mojibake. This catches DNS, certificate, and 1Panel reverse-proxy drift without adding credentials or production writes.

The latest production API health gate moves the manual health check into the same routine no-write P0 smoke. When the API target is `https://api.useskillhub.com`, `/health` must return the SkillHub gateway payload with `env=production`; the body is also scanned for sensitive output. This catches wrong reverse proxies, stale API containers, and accidental non-production boot modes before the deeper marketplace, account, MCP, and boundary checks run.
...
The latest workspace-write boundary smoke pass broadens the routine no-token production gate from read protection into representative mutation protection. It now sends write-shaped unauthenticated requests to publishing, skill feedback, trust reports, project subscriptions, organization team and webhook operations, billing profile updates, developer project and buyer-request creation, project save/install/policy/key/runtime actions, direct runtime invocation, pricing, publisher version submission, marketplace appeals, publisher feedback responses, publisher profile and terms, payout onboarding, and payout requests. Every route must stop at a JSON `401` or `403` boundary without leaking sensitive output, so routine 1Panel updates can catch accidentally writable Journey A/B/C controls without creating production rows.

The latest workspace-boundary smoke pass extends the no-token production gate from public/admin surfaces into the core signed-in workspaces. The routine public smoke now checks unauthenticated organization team, organization webhooks, organization billing, developer projects, publisher skills, publisher payouts, publisher finance ledger, and publisher marketplace appeals reads. Each endpoint must return a JSON `401` or `403` boundary with no sensitive-output leak, so a routine 1Panel update can catch accidental Journey A/B workspace exposure before the full protected admin gate is available.

The latest public detail support API boundary pass tightens the data behind Journey A skill detail. `/v1/skills/:slug/feedback` now returns a published-feedback projection instead of raw moderation rows, keeping rating, public text, use case, timestamps, and publisher responses while excluding reviewer email/display name, reviewer organization, project slug/id, moderation reason, operator notes, audit fields, tokens, and credentials. `/v1/skills/:slug/prices` now returns active public price rows only, leaving draft/archived prices, provider/customer state, commission splits, transactions, balances, payouts, invoices, and ledger metadata behind protected publisher/admin endpoints. The routine public smoke now enforces those boundary rules before a 1Panel update can pass.

The latest public skill-detail boundary pass makes the main Journey A inspection API safer. `/v1/skills/:slug` now returns a sanitized public manifest contract rather than the raw publisher manifest: embedded URL credentials are removed, local runtime command details are replaced with restricted placeholders, author URLs are credential-stripped, and secret handles are reduced to a count placeholder while preserving high-risk permission state. The routine public production smoke now fails if skill detail exposes project, operator, curation, invocation, billing, payout, audit, raw secret-handle, local-command, or embedded-credential details.

The latest public publisher boundary pass tightens the supplier-side trust API. The routine public production smoke now scans both `/v1/publishers` and `/v1/publishers/:slug` for private publisher/operator data: organization ids or members, billing/payment records, payout accounts or onboarding sessions, provider references, internal curation boost/reason, operator notes, audit rows, webhook secrets, raw tokens, API keys, credentials, and embedded URL credentials. Public `payoutStatus` remains allowed as a coarse readiness signal, so Journey A can show publisher trust without leaking Journey B/C operations.

The latest MCP resource-read hardening closes the other half of public agent discovery. Public `/mcp` `resources/list` now gets smoke coverage, and `resources/read` returns a sanitized public contract copy rather than the raw stored manifest: embedded URL credentials are removed, local command details and secret handles are reduced to safe public signals, and project, curation, operator, invocation, and billing state stay out of public MCP resources. The routine public production smoke parses the nested resource text so this boundary is checked before server updates.

The latest MCP public-gate pass tightens the agent entry to Journey A. Public `/mcp` `tools/list` now exposes marketplace-safe contract fields for discoverable skills: identity, input/output schemas, tags, semantic version, runtime type, and permission risk. The routine public production smoke verifies that the first public search skill is also discoverable through MCP, rejects project install/approval/curation/operator fields on public MCP tools, and sends an unauthenticated `tools/call` that must stop at `missing_api_key` without invocation, billable, or runtime output state.

The latest skill-detail QA pass tightens the Journey A customer path. The general smoke now checks the first real public skill detail page in English and Chinese for the developer handoff packet, feedback submission entry, and trust report entry, instead of only checking that the detail page returns HTTP 200 and includes the slug. The same pass expands mojibake detection with common UTF-8-as-GBK Chinese fragments, so a deployment cannot quietly ship corrupted skill-detail action copy or lose the feedback/trust controls while still passing the public page smoke.

The latest Journey A boundary pass makes those visible action controls safer. The routine public production gate now checks the detail support APIs for published feedback summaries and public price rows, then POSTs unauthenticated sample requests to skill feedback, skill trust-report, and project subscription creation endpoints and requires a `401` or `403` JSON error with no sensitive-output leak. This preserves the no-write 1Panel update workflow while proving that feedback moderation, trust reports, and subscription trials remain real authenticated operations rather than public form-shaped openings.

The latest deployment-runbook pass connects the P0 release gate to the real 1Panel update path. `docs/server-update.md` and `docs/1panel-deploy.md` now run `pnpm smoke:p0 -- --prod --skip-admin --timeout-ms 30000` as the routine no-write public production gate after health and public registry checks, then reserve the full protected Journey C gate for shells that already have an admin/super-admin user token configured. The runbooks also call out that mutating Journey A, Journey B, and demo-chain smokes create real marketplace, project, runtime, ledger, payout, notification, and audit rows and must remain planned rehearsals rather than routine deploy checks.

The latest runbook-QA pass makes that deployment guidance enforceable instead of relying on memory. The general smoke now checks `docs/server-update.md`, `docs/1panel-deploy.md`, and `docs/qa-smoke.md` before any network request, requiring the no-write public P0 gate and separate protected Journey C gate to stay documented while rejecting the old service-token-shaped `smoke:prod` server-update handoff. This keeps the user's 1Panel update workflow aligned with the same P0 release gate that protects marketplace discovery, bilingual pages, admin readiness shape, and secret-safe operations.

The latest release-command pass removes one more place where operators had to infer the correct command. `pnpm smoke:p0 -- --help` and `docs/api.md` now show the routine 1Panel public gate as `pnpm smoke:p0 -- --prod --skip-admin --timeout-ms 30000`, show the full protected Journey C gate separately as `pnpm smoke:p0 -- --prod --timeout-ms 30000`, and explain that the routine path performs no writes and needs no operator token. The general smoke now guards the release-suite help source, `docs/api.md`, and the deployment runbooks so CLI help, API reference, QA guide, and 1Panel handoff stay in lockstep.

The latest public-gate protection pass makes the routine no-token smoke more useful for Journey C risk. Even when the full admin smoke is skipped, the general production smoke now verifies that critical admin read endpoints for review, finance, payouts, notifications, notification templates, webhook delivery, identity, audit, and marketplace curation reject unauthenticated requests with 401 or 403 instead of leaking operator data.

The latest public publisher hardening makes the supplier-side proof chain more resilient during real deployment updates. `/v1/publishers` now detects whether the core public registry tables and optional operational tables exist before querying them. If public skills exist but `publisher_profiles` has not been migrated or backfilled yet, the API derives conservative pending publisher rows from public skill ownership and manifest authors. If optional review, pricing, install, or invocation tables are missing, the endpoint still returns real public publishers with default non-verified, free/draft, and zero metric signals, while launch readiness continues to expose the migration gap for operators.

The latest public search hardening applies the same rule to Journey A discovery. `/v1/skills/search`, public manifest listing, and public skill detail reads now depend on the core `skills` and `skill_versions` tables first, then add review, pricing, install, invocation, feedback, and curation signals only when those operational tables are present. A partially migrated deployment can still show real public skills with honest default signals, while admin launch readiness remains responsible for exposing the missing migration work.

The latest marketplace deep-link pass closes the remaining server-render discovery gap. `/v1/skills/search` now accepts the public `category` filter using the same tag-to-category mapping as the web marketplace, and `/marketplace` forwards recognized URL filters into that API before rendering the initial catalog. That means homepage handoff links and shared customer-demo URLs reproduce the same query, category, pricing, runtime, permission-risk, verification, and sort slice instead of depending on a default recommended list followed by client-only filtering.

The latest QA hardening expands the non-mutating general smoke from an English-first page check into a bilingual P0 surface check. The default app smoke now covers the core public and console pages in both English and Chinese, adds `/registry` to the default page set, checks the first real public skill detail and publisher profile in both language modes when public API rows exist, and requires stable Chinese product markers on the home, marketplace, publisher directory, publish, publisher, developer, dashboard, login, account, admin, docs, agents, registry, and terms pages. This protects the customer-demo path from a common failure where production APIs are healthy but a localized route, registry surface, or dynamic public detail page renders a 500, loses P0 markers, or shows mojibake only outside the English homepage.

The latest public-detail smoke hardening closes a narrower API gap in that same discovery chain. The general production smoke now verifies that the first real public skill returned by `/v1/skills/search` also returns a valid `/v1/skills/:slug` manifest with identity, runtime, permission, and schema fields, and that the first real public publisher returned by `/v1/publishers` also returns a `/v1/publishers/:slug` profile with trust, payout, metrics, and public skill rows. This catches a production drift where listing APIs are alive but detail APIs or profile hydration break before customers open a skill or publisher page.

The latest marketplace-list smoke hardening protects the first Journey A comparison surface from flattening into a static directory. When `/v1/skills/search` returns public skills, the general smoke now requires each row to carry the operating signals used by marketplace cards: version, verification state, permission risk, runtime type, billing model, install and call counts, success rate, latency, rating, and feedback count. It also fails if internal curation fields such as boost values or operator reasons appear in public discovery.

The latest runtime secret-safety pass closes a persistence risk in the invocation ledger. Runtime `input_summary` and `output_summary` now redact sensitive key names and authorization-shaped strings before storage, cap deep or long values, and the provider-disabled metered response no longer echoes raw invocation input into the output object that later becomes a summary.

The latest source-QA hardening adds a repository-level mojibake guard to the same general smoke. Before network checks run, the script scans web source files, QA scripts, and product docs for grouped historical encoding-corruption fragments such as replacement characters, the U+951F/U+65A4/U+62F7 replacement sequence, and UTF-8-as-GBK sequences. This protects the bilingual publish, publisher, developer, admin, shared-copy, and documentation surfaces from shipping corrupted Chinese source text even when a terminal display problem could otherwise hide the difference between real source damage and console rendering noise.

The latest publisher-directory hardening closes a visible public marketplace gap: production can have real public skills while `/v1/publishers` remains empty because no `publisher_profiles` rows exist yet. Public publisher reads now begin from real public skill ownership and approved/submitted/deprecated marketplace supply, then enrich with publisher profile and payout readiness when present. If a public skill exists before the publisher finishes profile setup, SkillHub exposes a conservative pending publisher row from the public manifest author/organization state, with `payoutStatus=not_configured` and no verified trust claim. Migration `031_public_publisher_profile_backfill.sql` creates the same pending profile rows for existing public supply, and future publish writes ensure a missing profile exists without overwriting publisher-edited records. The general smoke now fails when stats/search prove public supply but `/v1/publishers` returns none, and it checks the first public publisher profile page, so customer demos cannot silently lose the supplier-side proof chain again.

The latest production check found a real public-discovery failure pattern: app pages can return HTTP 200 while `/v1/skills/search` still fails or loses rows because the deployment database has not run every marketplace operations migration. Public registry reads now depend on the core `skills` and `skill_versions` tables first, then degrade optional review, pricing, install, invocation, feedback, and curation signals to conservative defaults when those tables are not present. Only missing core registry tables produce empty marketplace-safe public responses; internal operational probes, admin queues, and launch readiness still expose migration blockers. The migration runner now detects existing core SkillHub databases that are missing the early marketplace operations baseline and starts from `002_marketplace_operations.sql` without rerunning non-idempotent `001_initial.sql`, even if later migration history exists. The general smoke now checks `/v1/skills/search?limit=5`, fails when `/v1/stats` reports published supply but search is empty, and checks the first real public skill detail and publisher profile pages when those API rows exist, so this kind of "homepage works, marketplace/detail API broken" regression is caught before a customer demo.
...
The admin launch-readiness UI now makes those checks actionable. `/admin#launch-readiness` groups the five credibility thresholds into a customer-demo proof chain and gives every readiness row a secret-safe proof or fix target across marketplace, publishers, developer workspace, review, finance, payout, notification, webhook, identity, terms, audit, and docs surfaces. This closes an important demo credibility gap: operators can show where the evidence lives instead of only reading a count or blocker label.

The latest QA pass adds a P0 publish handoff smoke for the first serious Journey B -> C proof. It creates a generated publisher draft, submits the exact version for review, then verifies the publisher workspace row, publisher notification, admin review queue row, admin audit record, and admin notification queue using public/protected APIs rather than direct database checks. This gives the team a repeatable pre-demo signal that `/publish` is connected to real review, notification, and audit operations, not just a polished form.

The latest frontend QA hardening makes the public-to-backend handoff easier to protect before customer demos. The global sign-in action now lands directly on `/login`, `/admin-login` redirects to the normal login page without publishing an admin shortcut, and the non-mutating smoke checks assert that the home, login, dashboard, publish, publisher, developer, and admin pages still render their P0 operating markers. The smoke also fails on common Chinese mojibake markers, which prevents a deployment from looking technically alive while showing corrupted bilingual UI.

The latest developer handoff pass gives Journey A the same kind of repeatable proof as the publisher review smoke. Project skill installation and project API-key creation now write audit and notification records, and `pnpm smoke:p0:developer` proves marketplace discovery becomes saved project state, installed skill state, reveal-once key state, non-billable governed runtime test state, developer notifications, and admin audit visibility without querying the database directly.

The latest P0 smoke secret-safety pass makes the mutating Journey A and Journey B checks less trusting of their own protected reads. The developer handoff and publish handoff scripts now reuse the shared sensitive-output validator on protected project, publisher, notification, review, and audit responses, so raw user tokens, project API keys after first reveal, webhook secrets, provider keys, API-key fields, and email-code previews cannot slip through a passing segmented smoke. This keeps the focused P0 scripts aligned with the general launch-readiness and admin smoke secret-safety standard.

The latest demo-chain secret-safety pass brings the full customer-walkthrough smoke into that same standard. `pnpm smoke:p0:demo` now imports the shared sensitive-output validator and scans protected project detail, publisher/developer notifications, publisher/admin finance and payout reads, admin audit rows, and admin notification queues. The only expected raw project key remains the reveal-once creation response, so a later project detail, notification, ledger, payout, or audit leak fails the end-to-end smoke before a customer demo.

The latest admin operations pass gives Journey C its own repeatable proof. `pnpm smoke:p0:admin` is non-mutating and checks the admin page, protected admin API boundaries, launch readiness, review evidence, finance ledger and commission state, payout explainability, external delivery queues, webhook outbox, identity directory, audit logs, trust queues, and marketplace curation queues. It now scans protected overview, finance ledger, commission-rule, refund, dispute, and payout responses with the same sensitive-output validator used by launch readiness, review evidence, notifications, identity, and audit logs, so money-state inspection cannot pass while leaking credential-shaped values. It also rehearses the notification-delivery and webhook-outbox processors in `dry_run` mode, including a `fanoutMode: preview` summary for queued in-app events, email preferences, and organization webhook subscriptions without creating external rows or contacting providers. The same pass makes `/v1/admin/identity-directory` the canonical identity-directory endpoint while keeping `/v1/admin/identity` as a compatibility alias, so the page matrix, journey specs, API, and frontend are aligned.

The latest release-QA pass adds `pnpm smoke:p0` as a fixed-order P0 release gate. The default path runs public production-safe checks before non-mutating Journey C admin operations, so deployment preparation covers customer-facing pages and protected operator reads without requiring the team to remember multiple commands. Journey A, Journey B, and full demo-chain writes stay explicit opt-ins and still inherit the child scripts' production-write guards.

The latest launch-readiness QA pass protects that Journey C proof from quieter API drift and secret-safety regressions. The general smoke and `pnpm smoke:p0:admin` now share a contract validator that requires the readiness sections and customer-demo evidence item keys for identity, email, webhook, marketplace operations, launch credibility, commercial readiness, and production guardrails, plus a sensitive-output scanner for authorization-shaped strings, raw keys, and email-code previews. This does not pretend every environment is launch-ready; it makes sure the admin console keeps the evidence structure operators need to explain blockers, warnings, and proof targets before a customer walkthrough without exposing credentials.
...
The latest end-to-end demo pass connects the three P0 proofs into one customer-walkthrough smoke and now includes the money proof that was still too easy to hand-wave. `pnpm smoke:p0:demo` creates a generated publisher draft, submits the exact version, approves it through admin review, submits manual PayPal payout details, verifies payout readiness through a finance/admin operator path, activates a per-call price, verifies public marketplace discovery, creates a developer project, saves and installs the verified skill, creates a reveal-once project key, runs both console and MCP runtime invocations, processes the billable MCP call into a posted usage transaction, immutable split, and publisher balance, releases eligible balances, requests publisher payout, approves the payout, records the manual transfer reference, then checks project state, publisher/admin finance and payout ledgers, notifications, admin audit, and admin notification queues. This directly reduces the demo risk that SkillHub looks like several good pages rather than one operating marketplace loop.

Console access is now more product-visible without exposing admin entry on the public login page. The home and account surfaces show a role-aware map for `/login`, `/account`, `/developer`, `/publisher`, and `/admin`, including the required user roles, current session state, and the rule that production access uses account password, configured OAuth, or invite/recovery tokens rather than a shared backend password. This directly addresses customer-demo confusion where the platform looked like a public home page but the operating workspaces were not obvious.
...
The latest dashboard proof pass adds a P0 operating chain for customer walkthroughs. It compresses the end-to-end demo acceptance path into eight visible stages: publisher upload, version checks, admin review, marketplace listing, developer install, policy/key/runtime test, ledger/payout, and notification/readiness/audit proof. Each stage is derived from existing state and labeled as proof visible, needs attention, or waiting for data, which gives the team an honest demo-readiness map without introducing fake rows.

The latest account-entry pass makes `/login` and `/account` feel more like product workspaces instead of temporary authentication pages. `/login` now prioritizes Google/GitHub, username/email password login, and registration with workspace details, while token fallback remains secondary and admin entry is not advertised publicly. `/account` now summarizes identity, session security, workspace readiness, and operations readiness before the detailed panels, and its workspace shortcuts use role-aware states so customers can see why a console is available, sign-in-required, or role-required.
...
The latest marketplace discovery pass removes a small but visible fake-control risk from Journey A. The home registry search now submits into `/marketplace?q=...`, and the marketplace browser initializes and preserves query, category, pricing, permission-risk, runtime, verification, and sort filters in the URL. This makes customer demos and shared discovery links reproducible without adding a fake task API or exposing private ranking controls.
...
The latest review-handoff pass closes a quieter Journey B/C evidence gap. Submitting a skill version for review now records actor-aware audit state, returns the created automated-check summary, and queues an organization-scoped publisher notification; admin review decisions also write the deciding actor and notify the publisher organization. This makes upload-to-review visible in the same inbox/audit loops used by the rest of the platform instead of leaving submission as only a row in the review table.
...
The latest owner-approval pass closes a real Journey A governance gap. Saving a high-risk project skill policy with owner approval cleared now requires owner/admin authority, records the approver on the policy, synchronizes the installed-skill `approval_state` to `approved`, and writes audit plus notification records. High-risk installs therefore move from visible owner-review work into the same runtime gate that REST and MCP invocations enforce, instead of leaving the UI and gateway in different states.
...
- Dashboard publisher account operations now expose public publisher profile editing and manual PayPal/Alipay receiving-detail submission. Publishers can see the resulting readiness state and next action, while finance/admin operators record verification, blocking, and manual-transfer decisions.
...
- Email verification-code delivery decisions synchronize `email_login_challenges.delivery_status`, making signup/login support inspectable across SMTP, API provider, and manual retry paths.
...
- Provider-backed email sending is ready when SMTP (`SKILLHUB_EMAIL_PROVIDER=smtp`, SMTP host/user/password, and `SKILLHUB_EMAIL_FROM`) or Resend (`SKILLHUB_EMAIL_PROVIDER=resend`, `RESEND_API_KEY`, and `SKILLHUB_EMAIL_FROM`) is configured; missing provider configuration becomes an explicit failed state instead of a silent queue stall.
...
- Success states preserve the saved draft's exact version, expose whether it is a new or updated draft, let the publisher submit that version for review immediately, and route to the workspace, review repair, paid-readiness, and account/terms panels. Public skill detail remains a post-review verified/public listing path.
