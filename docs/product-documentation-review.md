...
The latest frontend QA hardening makes the public-to-backend handoff easier to protect before customer demos. The global sign-in action now lands directly on `/login`, `/admin-login` redirects to the login page's admin-entry section, and the non-mutating smoke checks assert that the home, login, dashboard, publish, publisher, developer, and admin pages still render their P0 operating markers. The smoke also fails on common Chinese mojibake markers, which prevents a deployment from looking technically alive while showing corrupted bilingual UI.
...
The latest account-entry pass makes `/login` and `/account` feel more like product workspaces instead of temporary authentication pages. `/login` now explains email-code readiness, Google/GitHub configuration state, token fallback boundaries, current session state, and the four post-login destinations before users reach the forms. `/account` now summarizes identity, session security, workspace readiness, and operations readiness before the detailed panels, and its workspace shortcuts use role-aware states so customers can see why a console is available, sign-in-required, or role-required.
