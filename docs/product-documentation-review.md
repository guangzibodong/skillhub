...
The latest admin operations pass gives Journey C its own repeatable proof. `pnpm smoke:p0:admin` is non-mutating and checks the admin page, protected admin API boundaries, launch readiness, review evidence, finance ledger and commission state, payout explainability, external delivery queues, webhook outbox, identity directory, audit logs, trust queues, and marketplace curation queues. The same pass makes `/v1/admin/identity-directory` the canonical identity-directory endpoint while keeping `/v1/admin/identity` as a compatibility alias, so the page matrix, journey specs, API, and frontend are aligned.

The latest end-to-end demo pass connects the three P0 proofs into one customer-walkthrough smoke. `pnpm smoke:p0:demo` creates a generated publisher draft, submits the exact version, approves it through admin review, verifies public marketplace discovery, creates a developer project, saves and installs the verified skill, creates a reveal-once project key, runs both console and MCP runtime invocations, then checks project state, notifications, admin audit, and admin notification queues. This directly reduces the demo risk that SkillHub looks like several good pages rather than one operating marketplace loop.
