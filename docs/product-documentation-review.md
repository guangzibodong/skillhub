...
The latest frontend QA hardening makes the public-to-backend handoff easier to protect before customer demos. The global `Console` / `后台入口` action now lands directly on `/dashboard#workspace-command-center`, and the non-mutating smoke checks assert that the home, login, dashboard, publish, publisher, developer, and admin pages still render their P0 operating markers. The smoke also fails on common Chinese mojibake markers, which prevents a deployment from looking technically alive while showing corrupted bilingual UI.
