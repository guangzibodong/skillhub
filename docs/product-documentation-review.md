...
The latest launch-readiness QA pass protects that Journey C proof from quieter API drift. The general smoke and `pnpm smoke:p0:admin` now share a contract validator that requires the readiness sections and customer-demo evidence item keys for identity, email, webhook, marketplace operations, launch credibility, commercial readiness, and production guardrails. This does not pretend every environment is launch-ready; it makes sure the admin console keeps the evidence structure operators need to explain blockers, warnings, and proof targets before a customer walkthrough.
