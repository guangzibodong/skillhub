...
The latest source-QA hardening adds a repository-level mojibake guard to the same general smoke. Before network checks run, the script scans web source files and QA scripts for grouped historical encoding-corruption fragments such as replacement characters, `锟斤拷`, and UTF-8-as-GBK sequences. This protects the bilingual publish, publisher, developer, admin, and shared-copy surfaces from shipping corrupted Chinese source text even when a terminal display problem could otherwise hide the difference between real source damage and console rendering noise.
