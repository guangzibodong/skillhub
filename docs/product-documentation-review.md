...
The latest admin operations pass gives Journey C its own repeatable proof. `pnpm smoke:p0:admin` is non-mutating and checks the admin page, protected admin API boundaries, launch readiness, review evidence, finance ledger and commission state, payout explainability, external delivery queues, webhook outbox, identity directory, audit logs, trust queues, and marketplace curation queues. It now also rehearses the notification-delivery and webhook-outbox processors in `dry_run` mode, including a `fanoutMode: preview` summary for queued in-app events, email preferences, and organization webhook subscriptions without creating external rows or contacting providers. The same pass makes `/v1/admin/identity-directory` the canonical identity-directory endpoint while keeping `/v1/admin/identity` as a compatibility alias, so the page matrix, journey specs, API, and frontend are aligned.
