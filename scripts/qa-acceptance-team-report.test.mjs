...
test("admin workspace opens as an operations console", async () => {
  ...
  assert.match(adminPage, /className="admin-console-shell admin-console-shell--v2"/);
  assert.match(adminPage, /signOutAction/);
  assert.match(adminPage, /className="admin-operator-topbar"/);
  assert.match(adminPage, /className="admin-global-search admin-global-search--status"/);
  assert.match(adminPage, /className="admin-command-grid"/);
  assert.match(adminPage, /className="admin-command-hero"/);
  assert.match(adminPage, /className="admin-sidebar-nav"/);
  assert.match(adminPage, /className="admin-sidebar-nav__group"/);
  ...
  assert.doesNotMatch(adminPage, /adminV2Labels\.selectedOrder\.actions/);
  assert.match(adminPage, /adminConsoleLabels\.shell\.timeRanges/);
  assert.match(adminPage, /adminV2Labels\.orders\.tabs/);
  ...
  assert.match(adminPage, /adminConsoleLabels\.traffic\.title/);
  assert.match(adminPage, /id="admin-traffic"/);
  assert.match(adminPage, /className="admin-motion-chart"/);
  assert.match(adminPage, /className="admin-chart-line admin-chart-line--gmv"/);
  assert.match(adminPage, /className="admin-order-table"/);
  assert.match(adminPage, /className="admin-payment-card-grid"/);
  assert.match(adminPage, /className="admin-traffic-workspace"/);
  assert.match(adminPage, /className="admin-selected-order"/);
  assert.match(adminPage, /Stripe checkout/);
  assert.match(adminPage, /Alipay/);
  assert.match(adminPage, /Cloudflare analytics/);
  assert.match(adminPage, /UV/);
  assert.match(adminPage, /Search order ID/);
  assert.match(adminPage, /GMV/);

  assert.match(globals, /\.admin-console-shell/);
  assert.match(globals, /\.admin-console-shell--v2/);
  assert.match(globals, /\.admin-operator-topbar/);
  assert.match(globals, /\.admin-global-search/);
  assert.match(globals, /\.admin-global-search--status/);
  assert.match(globals, /\.admin-command-grid/);
  assert.match(globals, /\.admin-command-hero/);
  assert.match(globals, /\.admin-sidebar-nav/);
  assert.match(globals, /\.admin-sidebar-nav__group/);
  assert.match(globals, /\.admin-status-strip/);
  ...
  assert.match(globals, /\.admin-motion-chart/);
  assert.match(globals, /\.admin-chart-line/);
  assert.match(globals, /@keyframes admin-chart-draw/);
  assert.match(globals, /@keyframes admin-chart-scan/);
  assert.match(globals, /@keyframes admin-chart-points/);
  assert.match(globals, /@keyframes admin-chart-glow/);
  assert.match(globals, /@keyframes admin-bar-rise/);
  assert.match(globals, /@keyframes admin-risk-pulse/);
  assert.match(globals, /\.admin-order-table/);
  assert.match(globals, /\.admin-payment-card-grid/);
  assert.match(globals, /\.admin-traffic-workspace/);
  assert.match(globals, /\.admin-selected-order/);
  assert.match(globals, /prefers-reduced-motion: reduce/);
